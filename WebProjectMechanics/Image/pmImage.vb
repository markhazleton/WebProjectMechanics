...
Public Class pmImage
    ...
    Sub New()
        ...
        OverlayText = "This is the Default Text"
    End Sub
    ...
    End Function
    Public Function GetContentType(ext As String) As String
        ...
        If OverlayText > String.Empty AndAlso PercentFill > 0 Then
            ...
            Select Case Position
                Case ContentAlignment.TopLeft ' =1
                    rect = New RectangleF(f.Size * 0.15!,
                        (f.Size * 0.1!),
                        layout.Width,
                        CSng(img.Height) * SQRTFill)
                Case ContentAlignment.TopCenter ' =2
                    rect = New RectangleF((bmp.Width - s.Width) / 2,
                        (f.Size * 0.1!),
                        layout.Width,
                        CSng(img.Height) * SQRTFill)
                Case ContentAlignment.TopRight ' =4
                    rect = New RectangleF((bmp.Width - s.Width) - (f.Size * 0.1!),
                        (f.Size * 0.1!),
                        layout.Width,
                        CSng(img.Height) * SQRTFill)
                Case ContentAlignment.MiddleLeft ' =16  huh?  where's 8?
                    rect = New RectangleF(f.Size * 0.15!,
                        (bmp.Height - s.Height) / 2,
                        layout.Width,
                        CSng(img.Height) * SQRTFill)
                ...
                Case ContentAlignment.BottomLeft ' =256  and 128?
                    rect = New RectangleF(f.Size * 0.15!,
                        (bmp.Height - s.Height) - (f.Size * 0.1!),
                        layout.Width,
                        CSng(img.Height) * SQRTFill)
