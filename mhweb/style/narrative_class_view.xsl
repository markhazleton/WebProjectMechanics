...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template match="/">
		<xsl:for-each select="ppmsNarrative">
			<table class="NarrativeTable" cellspacing="5" cellpadding="5" width="725">
				<tbody>
					...
					<tr>
						<td rowspan="2">
							<img src="/app_themes/pm/logo.gif" width="109" height="75" alt="Project Mechanics"/>
						</td>
						<td class="NarrativeValue" width="355" colspan="3">
							<strong>
								<xsl:value-of select="NarrativeNM"/>-<xsl:value-of select="NarrativeTypeNM"/></strong>
						</td>
					</tr>
					<tr>
					...
					<tr>
						...
						<td class="NarrativeValue" colspan="3">
							<xsl:value-of select="RevisedByContactNM"/>
						</td>
					</tr>
					<tr>
						<td class="NarrativeLabel">Department:</td>
						<td class="NarrativeValue" colspan="3">
							<xsl:value-of select="FunctionalGroupNM"/>
						</td>
					</tr>
					<tr>
						<td class="NarrativeLabel">Effective Date:</td>
						<td class="NarrativeValue" colspan="3">
							<xsl:value-of select="VersionEffectiveDT"/>
						</td>
					</tr>
					<tr>
					...
					<xsl:if test="Statement!=''">
						<tr>
							...
							<td class="NarrativeValue" colspan="3">
								<xsl:value-of select="Statement" disable-output-escaping="no"/>
								<hr/>
							</td>
						</tr>
					</xsl:if>
					<xsl:for-each select="NarrativeSteps/ppmsNarrativeStep">
						<tr>
							<td class="NarrativeLabel">
								<xsl:value-of select="StepNarrativeNM" disable-output-escaping="no"/>
								<br/>
							</td>
							<td class="NarrativeValue" colspan="3">
								<xsl:value-of select="StepNarrative" disable-output-escaping="no"/>
								<br/>
								<hr/>
							</td>
						</tr>
					...
					<tr>
						...
						<td class="NarrativeValue" colspan="3">
							<xsl:for-each select="NarrativeLinks/NarrativeLink">
								<a>
					...
					<tr>
						<td class="NarrativeLabel">Relevant Procedures:</td>
						<td class="NarrativeValue" colspan="3">
							<xsl:if test="SecurityGroupID=ParentSecurityGroupID">
								<xsl:if test="ParentNarrativeNM!=''">
									<a>
										<xsl:attribute name="ID">a_<xsl:value-of select="NarrativeID"/></xsl:attribute>
										<xsl:attribute name="href">ppms.aspx?url=<xsl:value-of select="ParentNarrativeFileName"/></xsl:attribute>
										<xsl:value-of select="ParentNarrativeNM"/>-<xsl:value-of select="ParentNarrativeTypeNM"/></a>
									<br/>
									<hr/>
								</xsl:if>
							</xsl:if>
							<xsl:for-each select="RelatedNarratives">
								<xsl:if test="../SecurityGroupID=SecurityGroupID">
