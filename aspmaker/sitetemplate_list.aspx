...
<script runat="server">
	...
	Dim key As SiteTemplatekey = New SiteTemplatekey()  ' record key
	...
	Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs)
		...
		tableId = SiteTemplateGridView.ClientID
	...
	Protected Sub Page_PreRender(ByVal sender As Object, ByVal e As System.EventArgs)
		If SiteTemplateGridView.Rows.Count = 0 Then
	...
	Private Function ewRecordCount() As Integer
		...
		Try
			...
			'If (iRecordCount = 0)
				Dim data As SiteTemplatedal = New SiteTemplatedal()
				iRecordCount = data.GetRowsCount(SiteTemplateinf.GetUserFilter()) 
	...
	Protected Sub Sorting(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewSortEventArgs)
		...
		Dim oInfo As SiteTemplateinf = New SiteTemplateinf()
		...
		Do While (i <= oInfo.TableInfo.Count)
			...
			Dim fldProfile As FieldProfile = CustomProfile.GetField(Share.ProjectName, SiteTemplateinf.TableVar, sSortParm)
	...
	Protected Sub SiteTemplateGridView_PageIndexChanged(ByVal sender As Object,ByVal e As System.EventArgs)
		objProfile.PageIndex = SiteTemplateGridView.PageIndex ' Save page index
	End Sub
	...
	Protected Sub SiteTemplateGridView_RowCreated(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewRowEventArgs)
			...
			For each cell As TableCell In e.Row.Cells
				If (cell.HasControls()) Then
					If (cell.Controls.Count > 2 AndAlso (TypeOf cell.Controls(1) is LinkButton)) Then
						...
						Dim sSort As String = CustomProfile.GetField(Share.ProjectName, SiteTemplateinf.TableVar, button.CommandArgument).Sort
	...
	Private Sub ResetPageProperties()
		...
		' Clear Page Index
		objProfile.PageIndex = 0
		SiteTemplateGridView.PageIndex = 0
	End Sub
	...
	Protected Sub BindData()
		...
		If (objProfile.PageIndex < 0) Then
			objProfile.PageIndex = 0
		End If
		SiteTemplateGridView.AllowPaging = objProfile.AllowPaging
		If (objProfile.PageSize > 0) Then
			SiteTemplateGridView.PageSize = objProfile.PageSize 
		End If
		If (Not objProfile.IsCollapsed) Then

			' Try to restore page index
			Try
				Dim intPageCount As Integer = data.GetPageCount(sWhere, SiteTemplateGridView.PageSize)
				If (SiteTemplateGridView.PageSize > 0 AndAlso intPageCount < objProfile.PageIndex + 1) Then objProfile.PageIndex = IIf(intPageCount > 0, intPageCount - 1, 0)
				SiteTemplateGridView.PageIndex = objProfile.PageIndex
			Catch e As Exception
				ResetPageProperties()
			End Try
		Else
	...
	Protected Sub SiteTemplateGridView_Init(ByVal sender As Object, ByVal e As System.EventArgs)
		Try
	...
	Protected Sub SiteTemplateGridView_Load(ByVal sender As Object, ByVal e As System.EventArgs)
		If (isExport) Then
			SetupControlsVisible(False)
			SiteTemplateGridView.PagerSettings.Visible = False
			Return
		End If
	End Sub
	...
	Protected Sub SiteTemplateGridView_Unload(ByVal sender As Object, ByVal e As System.EventArgs)
		Try
	...
	Protected Sub SiteTemplateGridView_RowDataBound(ByVal sender As Object, ByVal e As GridViewRowEventArgs)
		tableId = TryCast(sender, GridView).ClientID
		If (SiteTemplateGridView.PageCount > 1 AndAlso SiteTemplateGridView.PagerSettings.Visible) Then
			If (SiteTemplateGridView.PagerSettings.Position = PagerPosition.Top OrElse SiteTemplateGridView.PagerSettings.Position = PagerPosition.TopAndBottom) Then
				firstRowOffset = initialFirstRowOffset + 1
			End If
			If (SiteTemplateGridView.PagerSettings.Position = PagerPosition.Bottom OrElse SiteTemplateGridView.PagerSettings.Position = PagerPosition.TopAndBottom) Then
				lastRowOffset = initialLastRowOffset + 1
			End If
		End If
		If (e.Row.RowType = DataControlRowType.Header) Then
		ElseIf (e.Row.RowType = DataControlRowType.DataRow) Then
			Dim row As SiteTemplaterow = TryCast(e.Row.DataItem, SiteTemplaterow) ' get row object
			...
			If (EditLink IsNot Nothing) Then _
				EditLink.NavigateUrl = String.Format("sitetemplate_edit.aspx?TemplatePrefix={0}", Server.UrlEncode(Convert.ToString(row.TemplatePrefix)))
			Dim CopyLink As HyperLink = TryCast(control.FindControl("CopyLink"), HyperLink)
			If (CopyLink IsNot Nothing) Then _
				CopyLink.NavigateUrl = String.Format("sitetemplate_add.aspx?TemplatePrefix={0}", Server.UrlEncode(Convert.ToString(row.TemplatePrefix)))
			Dim DeleteLink As HyperLink = TryCast(control.FindControl("DeleteLink"), HyperLink)
			If (DeleteLink IsNot Nothing) Then _
				DeleteLink.NavigateUrl = String.Format("sitetemplate_delete.aspx?TemplatePrefix={0}", Server.UrlEncode(Convert.ToString(row.TemplatePrefix)))
	...
	Protected Sub SiteTemplateGridView_DataBound(ByVal sender As Object, ByVal e As System.EventArgs)
	End Sub
	...
	Protected Sub SiteTemplateGridView_RowCommand(ByVal sender As Object, ByVal e As GridViewCommandEventArgs)
	End Sub
	...
	Private Sub SetupControlsVisible(ByVal visible As Boolean)
		SiteTemplateGridView.Columns(0).Visible = visible
		SiteTemplateGridView.Columns(1).Visible = visible
		lnkAdd.Visible = visible
		SiteTemplateGridView.Columns(2).Visible = visible
		SiteTemplateGridView.AllowSorting = visible
	End Sub
	...
	Public Sub Export_Command(Byval exportType As String)
		...
		Select Case exportType.ToUpper()
			Case "EXCEL"
				...
				Response.AddHeader("Content-Disposition", "attachment; filename=""SiteTemplate.xls""")
				Export("excel")
			Case "XML"
				Response.Clear()
				Response.ContentType = "text/xml"
				Response.AddHeader("Content-Disposition", "attachment; filename=""SiteTemplate.xml""")
				Export("xml")
		End Select
	End Sub
	...
	Private Sub Export(ByVal sExport As String)
		...
		If (isExportWord OrElse isExportExcel) Then
			Using sw As StringWriter = New StringWriter()
				Using htw As HtmlTextWriter = New HtmlTextWriter(sw)
					SiteTemplateGridView.Columns(0).Visible = False
					SiteTemplateGridView.Columns(1).Visible = False
					lnkAdd.Visible = False
					SiteTemplateGridView.Columns(2).Visible = False
					SiteTemplateGridView.PagerSettings.Visible = False
					SiteTemplateGridView.AllowSorting = False
					BindData()
					PrepareGridViewForExport(SiteTemplateGridView)
					...
					htw.WriteEncodedText("TABLE: Site Template")
					htw.RenderEndTag()

					' Render Table
					SiteTemplateGridView.RenderControl(htw)
		...
		If (isExportCsv OrElse isExportXml) Then
			...
			Dim dataSet As SiteTemplaterows 
			...
			End If
			Dim data As SiteTemplatedal = New SiteTemplatedal()
			dataSet = TryCast(data.LoadList(SiteTemplateinf.GetUserFilter()), SiteTemplaterows)
			...
			If (isExportXml) Then
				...
				Do While (dataSet IsNot Nothing AndAlso i < dataSet.Count AndAlso (nStartRec < nStopRec))
					...
					If (dataSet(i).Name IsNot Nothing) Then
						oXmlField.InnerText = Convert.ToString(dataSet(i).Name)
					Else
...
<asp:ScriptManager ID="ScriptManager1" runat="server" />
	<span class="aspnetmaker">Site Template</span>
	<asp:PlaceHolder ID="pnlExport" runat="server">
		<asp:HyperLink CssClass="aspnetmaker" ID="lnkExportExcel" runat="server" NavigateUrl="sitetemplate_list.aspx?Export=excel">Export to Excel</asp:HyperLink>&nbsp;&nbsp;
		<asp:HyperLink CssClass="aspnetmaker" ID="lnkExprotXML" runat="server" NavigateUrl="sitetemplate_list.aspx?Export=xml">Export to XML</asp:HyperLink>&nbsp;&nbsp;
	</asp:PlaceHolder>
...
//-->
	...
	<asp:ValidationSummary id="xevs_SiteTemplate" CssClass="aspnetmaker" runat="server"
		...
		ShowMessageBox="True"
		ForeColor="#FF0000" />
	<p><asp:HyperLink ID="lnkAdd" runat="server" CssClass="aspnetmaker" NavigateUrl="sitetemplate_add.aspx">Add</asp:HyperLink></p>
	<asp:PlaceHolder ID="pnlMessage" runat="server" Visible="false">
		<p><asp:Label id="lblMessage" forecolor="#FF0000" CssClass="aspnetmaker" runat="server" /></p>
	</asp:PlaceHolder>
	<p><asp:Label id="lblReturnUrl" Visible="False" Text="sitetemplate_list.aspx" CssClass="aspnetmaker" runat="server" />
...
<asp:ObjectDataSource ID="SiteTemplateDataSource"
	...
	<asp:GridView ID="SiteTemplateGridView"
		PageSize="50"
		...
		OnInit="SiteTemplateGridView_Init"
		OnDataBound="SiteTemplateGridView_DataBound"
		OnRowCommand="SiteTemplateGridView_RowCommand"
		OnRowDataBound="SiteTemplateGridView_RowDataBound"
		OnSorting="Sorting"
		OnRowCreated="SiteTemplateGridView_RowCreated"
		OnPageIndexChanged="SiteTemplateGridView_PageIndexChanged"
		OnLoad="SiteTemplateGridView_Load"
		OnUnload="SiteTemplateGridView_Unload"
		...
		<asp:TemplateField>
			...
			<asp:TemplateField>
				...
				<HeaderTemplate>
					<asp:LinkButton runat='server' id="xs_Name"  CssClass="ewTableHeader" CommandName="Sort" CommandArgument="Name">Name </asp:LinkButton>
				</HeaderTemplate>
				<ItemTemplate>
