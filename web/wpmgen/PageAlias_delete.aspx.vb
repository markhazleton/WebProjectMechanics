...
Partial Class PageAlias_delete
	...
	Sub Page_Main()
		...
		For i As Integer = 0 to arRecKeys.GetUpperBound(0)
			...
			sFilter = sFilter & "[PageAliasID]=" & ew_AdjustSql(sKeyFld) & " AND "
    ...
    user = CurrentUserName()
		...
		' PageAliasID Field
		ew_WriteAuditTrail(filePfx, curDate, curTime, id, user, action, table, "PageAliasID", keyvalue, RsSrc("PageAliasID"), newvalue)

		' PageURL Field
		ew_WriteAuditTrail(filePfx, curDate, curTime, id, user, action, table, "PageURL", keyvalue, RsSrc("PageURL"), newvalue)

		' TargetURL Field
		ew_WriteAuditTrail(filePfx, curDate, curTime, id, user, action, table, "TargetURL", keyvalue, RsSrc("TargetURL"), newvalue)

		' AliasType Field
		ew_WriteAuditTrail(filePfx, curDate, curTime, id, user, action, table, "AliasType", keyvalue, RsSrc("AliasType"), newvalue)
