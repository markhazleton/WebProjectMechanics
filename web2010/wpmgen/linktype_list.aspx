<%@ Page ClassName="linktype_list" Language="VB" MasterPageFile="masterpage.master" ValidateRequest="false" AutoEventWireup="false" CodeFile="linktype_list.aspx.vb" Inherits="linktype_list" CodeFileBaseClass="AspNetMaker8_wpmWebsite" %>
...
LinkType_list.FormID = "fLinkTypelist"; // form ID 
var EW_PAGE_ID = LinkType_list.PageID; // for backward compatibility
// extend page with Form_CustomValidate function
LinkType_list.Form_CustomValidate =  
...
Rs = LinkType_list.LoadRecordset()
	...
	End If
%>
<p><span class="aspnetmaker" style="white-space: nowrap;"><%= Language.Phrase("TblTypeTABLE") %><%= LinkType.TableCaption %>
<% If LinkType.Export = "" AndAlso LinkType.CurrentAction = "" Then %>
&nbsp;&nbsp;<a href="<%= LinkType_list.ExportExcelUrl %>"><img src='images/exportxls.gif' alt='<%= ew_HtmlEncode(Language.Phrase("ExportToExcel")) %>' title='<%= ew_HtmlEncode(Language.Phrase("ExportToExcel")) %>' width='16' height='16' border='0'></a>
<% End If %>
</span></p>
<% If LinkType.Export = "" AndAlso LinkType.CurrentAction = "" Then %>
<a href="javascript:ew_ToggleSearchPanel(LinkType_list);" style="text-decoration: none;"><img id="LinkType_list_SearchImage" src="images/collapse.gif" alt="" width="9" height="9" border="0"></a><span class="aspnetmaker">&nbsp;<%= Language.Phrase("Search") %></span><br>
...
<table class="ewBasicSearch">
	<tr>
		<td><span class="aspnetmaker">
			<a href="<%= LinkType_list.PageUrl %>cmd=reset"><%= Language.Phrase("ShowAll") %></a>&nbsp;
			<a href="linktype_srch.aspx"><%= Language.Phrase("AdvancedSearch") %></a>&nbsp;
		</span></td>
	</tr>
</table>
</form>
...
<% End If %>
<% If EW_DEBUG_ENABLED Then HttpContext.Current.Response.Write(LinkType_list.DebugMsg) %>
<% LinkType_list.ShowMessage() %>
<br />
<table cellspacing="0" class="ewGrid"><tr><td class="ewGridContent">
<form name="fLinkTypelist" id="fLinkTypelist" class="ewForm" method="post">
<div id="gmp_LinkType" class="ewGridMiddlePanel">
...
<%
		...
		' Render list options
		LinkType_list.RenderListOptions()

		' Render list options (header, left)
		LinkType_list.ListOptions.Render("header", "left")
%>
<% If LinkType.LinkTypeCD.Visible Then ' LinkTypeCD %>
	<% If LinkType.SortUrl(LinkType.LinkTypeCD) = "" Then %>
		<td><%= LinkType.LinkTypeCD.FldCaption %></td>
	<% Else %>
		<td><div class="ewPointer" onmousedown="ew_Sort(event,'<%= LinkType.SortUrl(LinkType.LinkTypeCD) %>',1);">
			<table cellspacing="0" class="ewTableHeaderBtn"><thead><tr><td><%= LinkType.LinkTypeCD.FldCaption %></td><td style="width: 10px;"><% If LinkType.LinkTypeCD.Sort = "ASC" Then %><img src="images/sortup.gif" width="10" height="9" border="0"><% ElseIf LinkType.LinkTypeCD.Sort = "DESC" Then %><img src="images/sortdown.gif" width="10" height="9" border="0"><% End If %></td></tr></thead></table>
		</div></td>
	<% End If %>
<% End If %>		
<% If LinkType.LinkTypeDesc.Visible Then ' LinkTypeDesc %>
	<% If LinkType.SortUrl(LinkType.LinkTypeDesc) = "" Then %>
		<td><%= LinkType.LinkTypeDesc.FldCaption %></td>
	<% Else %>
		<td><div class="ewPointer" onmousedown="ew_Sort(event,'<%= LinkType.SortUrl(LinkType.LinkTypeDesc) %>',1);">
			<table cellspacing="0" class="ewTableHeaderBtn"><thead><tr><td><%= LinkType.LinkTypeDesc.FldCaption %></td><td style="width: 10px;"><% If LinkType.LinkTypeDesc.Sort = "ASC" Then %><img src="images/sortup.gif" width="10" height="9" border="0"><% ElseIf LinkType.LinkTypeDesc.Sort = "DESC" Then %><img src="images/sortdown.gif" width="10" height="9" border="0"><% End If %></td></tr></thead></table>
		</div></td>
	<% End If %>
<% End If %>		
<% If LinkType.LinkTypeTarget.Visible Then ' LinkTypeTarget %>
	<% If LinkType.SortUrl(LinkType.LinkTypeTarget) = "" Then %>
		<td><%= LinkType.LinkTypeTarget.FldCaption %></td>
	<% Else %>
		<td><div class="ewPointer" onmousedown="ew_Sort(event,'<%= LinkType.SortUrl(LinkType.LinkTypeTarget) %>',1);">
			<table cellspacing="0" class="ewTableHeaderBtn"><thead><tr><td><%= LinkType.LinkTypeTarget.FldCaption %></td><td style="width: 10px;"><% If LinkType.LinkTypeTarget.Sort = "ASC" Then %><img src="images/sortup.gif" width="10" height="9" border="0"><% ElseIf LinkType.LinkTypeTarget.Sort = "DESC" Then %><img src="images/sortdown.gif" width="10" height="9" border="0"><% End If %></td></tr></thead></table>
		</div></td>
	<% End If %>
<% End If %>		
<%
		...
		LinkType_list.ListOptions.Render("header", "right")
%>
	</tr>
</thead>
<tbody><!-- Table body -->
...
LinkType.RowType = EW_ROWTYPE_AGGREGATEINIT
...
Do While (LinkType.CurrentAction = "gridadd" OrElse Rs.Read()) AndAlso (LinkType_list.lRecCnt < LinkType_list.lStopRec)
	...
	LinkType.CssClass = ""
	LinkType.CssStyle = ""
	LinkType.RowAttrs.Clear()
	ew_SetAttr(LinkType.RowAttrs, New String(){"onmouseover", "onmouseout", "onclick"}, New String(){"ew_MouseOver(event, this);", "ew_MouseOut(event, this);", "ew_Click(event, this);"})
...
<%
		...
		LinkType_list.ListOptions.Render("body", "left")
%>
	<% If LinkType.LinkTypeCD.Visible Then ' LinkTypeCD %>
...
<%
		...
		LinkType_list.ListOptions.Render("body", "right")
%>
	</tr>
<%
	End If
...
<!--first page button-->
	...
	<td><a href="<%= LinkType_list.PageUrl %>start=<%= LinkType_list.Pager.FirstButton.Start %>"><img src="images/first.gif" alt="<%= Language.Phrase("PagerFirst") %>" width="16" height="16" border="0"></a></td>
	<% Else %>
...
<!--previous page button-->
	...
	<td><a href="<%= LinkType_list.PageUrl %>start=<%= LinkType_list.Pager.PrevButton.Start %>"><img src="images/prev.gif" alt="<%= Language.Phrase("PagerPrevious") %>" width="16" height="16" border="0"></a></td>
	<% Else %>
...
<!--next page button-->
	...
	<td><a href="<%= LinkType_list.PageUrl %>start=<%= LinkType_list.Pager.NextButton.Start %>"><img src="images/next.gif" alt="<%= Language.Phrase("PagerNext") %>" width="16" height="16" border="0"></a></td>
	<% Else %>
...
<!--last page button-->
	...
	<td><a href="<%= LinkType_list.PageUrl %>start=<%= LinkType_list.Pager.LastButton.Start %>"><img src="images/last.gif" alt="<%= Language.Phrase("PagerLast") %>" width="16" height="16" border="0"></a></td>	
	<% Else %>
	<td><img src="images/lastdisab.gif" alt="<%= Language.Phrase("PagerLast") %>" width="16" height="16" border="0"></td>
	<% End If %>
	<td><span class="aspnetmaker">&nbsp;<%= Language.Phrase("Of") %>&nbsp;<%= LinkType_list.Pager.PageCount %></span></td>
	</tr></table>
	</td>	
	<td>&nbsp;&nbsp;&nbsp;&nbsp;</td>
	<td>
	<span class="aspnetmaker"><%= Language.Phrase("Record") %>&nbsp;<%= LinkType_list.Pager.FromIndex %>&nbsp;<%= Language.Phrase("To") %>&nbsp;<%= LinkType_list.Pager.ToIndex %>&nbsp;<%= Language.Phrase("Of") %>&nbsp;<%= LinkType_list.Pager.RecordCount %></span>
<% Else %>
...
<option value="20"<% If LinkType_list.lDisplayRecs = 20 Then %> selected="selected"<% End If %>>20</option>
<option value="40"<% If LinkType_list.lDisplayRecs = 40 Then %> selected="selected"<% End If %>>40</option>
<option value="60"<% If LinkType_list.lDisplayRecs = 60 Then %> selected="selected"<% End If %>>60</option>
<option value="ALL"<% If LinkType.RecordsPerPage = -1 Then %> selected="selected"<% End If %>><%= Language.Phrase("AllRecords") %></option>
</select></td></tr></table>
		</td>
<% End If %>
	</tr>
...
<a href="<%= LinkType_list.AddUrl %>"><%= Language.Phrase("AddLink") %></a>&nbsp;&nbsp;
</div>
</div>
<% End If %>
</td></tr></table>
