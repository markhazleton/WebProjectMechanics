...
Partial Class linktype_delete
	...
	Class cLinkType_delete
		...
		' ASP.NET page object
		Public ReadOnly Property AspNetPage() As linktype_delete
			Get
				Return CType(m_ParentPage, linktype_delete)
			End Get
		End Property
	...
	Sub Page_Main()
		...
		For i As Integer = 0 to arRecKeys.GetUpperBound(0)
			...
			' Set up key field
			sKeyFld = sKey
			sFilter &= "[LinkTypeCD]='" & ew_AdjustSql(sKeyFld) & "' AND "
    ...
    user = CurrentUserName()
		...
		' LinkTypeDesc Field
		ew_WriteAuditTrail(filePfx, curDateTime, id, user, action, table, "LinkTypeDesc", keyvalue, RsSrc("LinkTypeDesc"), newvalue)

		' LinkTypeComment Field
		ew_WriteAuditTrail(filePfx, curDateTime, id, user, action, table, "LinkTypeComment", keyvalue, "<MEMO>", newvalue)

		' LinkTypeTarget Field
		ew_WriteAuditTrail(filePfx, curDateTime, id, user, action, table, "LinkTypeTarget", keyvalue, RsSrc("LinkTypeTarget"), newvalue)
	End Sub
