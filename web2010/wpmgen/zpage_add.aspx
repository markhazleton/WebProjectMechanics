<%@ Page ClassName="zpage_add" Language="VB" MasterPageFile="masterpage.master" ValidateRequest="false" AutoEventWireup="false" CodeFile="zpage_add.aspx.vb" Inherits="zpage_add" CodeFileBaseClass="AspNetMaker8_wpmWebsite" %>
...
zPage_add.FormID = "fzPageadd"; // form ID 
...
<p><span class="aspnetmaker"><%= Language.Phrase("Add") %>&nbsp;<%= Language.Phrase("TblTypeTABLE") %><%= zPage.TableCaption %><br /><br />
<a href="<%= zPage.ReturnUrl %>"><%= Language.Phrase("GoBack") %></a></span></p>
<% If EW_DEBUG_ENABLED Then HttpContext.Current.Response.Write(zPage_add.DebugMsg) %>
...
<% Else %>
<% zPage.CompanyID.EditAttrs("onchange") = "ew_UpdateOpt('x_ParentPageID','x_CompanyID',zPage_add.ar_x_ParentPageID);" %>
