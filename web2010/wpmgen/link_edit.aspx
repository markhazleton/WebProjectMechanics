<%@ Page ClassName="link_edit" Language="VB" MasterPageFile="masterpage.master" ValidateRequest="false" AutoEventWireup="false" CodeFile="link_edit.aspx.vb" Inherits="link_edit" CodeFileBaseClass="AspNetMaker8_wpmWebsite" %>
...
Link_edit.FormID = "fLinkedit"; // form ID 
...
<p><span class="aspnetmaker"><%= Language.Phrase("Edit") %>&nbsp;<%= Language.Phrase("TblTypeTABLE") %><%= Link.TableCaption %><br /><br />
<a href="<%= Link.ReturnUrl %>"><%= Language.Phrase("GoBack") %></a></span></p>
<% If EW_DEBUG_ENABLED Then HttpContext.Current.Response.Write(Link_edit.DebugMsg) %>
<% Link_edit.ShowMessage() %>
<form name="fLinkedit" id="fLinkedit" method="post" onsubmit="this.action=location.pathname;return Link_edit.ValidateForm(this);">
<p />
...
<% Else %>
<% Link.CompanyID.EditAttrs("onchange") = "ew_UpdateOpt('x_zPageID','x_CompanyID',Link_edit.ar_x_zPageID);" %>
...
<script language="JavaScript" type="text/javascript">
<!--
ew_UpdateOpts([['x_zPageID','x_CompanyID',Link_edit.ar_x_zPageID]]);
//-->
</script>
<script language="JavaScript" type="text/javascript">
<!--
