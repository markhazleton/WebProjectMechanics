<%@ Page ClassName="linkcategory_edit" Language="VB" MasterPageFile="masterpage.master" ValidateRequest="false" AutoEventWireup="false" CodeFile="linkcategory_edit.aspx.vb" Inherits="linkcategory_edit" CodeFileBaseClass="AspNetMaker8_wpmWebsite" %>
...
LinkCategory_edit.FormID = "fLinkCategoryedit"; // form ID 
...
LinkCategory_edit.Form_CustomValidate =  
...
<p><span class="aspnetmaker"><%= Language.Phrase("Edit") %>&nbsp;<%= Language.Phrase("TblTypeTABLE") %><%= LinkCategory.TableCaption %><br /><br />
<a href="<%= LinkCategory.ReturnUrl %>"><%= Language.Phrase("GoBack") %></a></span></p>
<% If EW_DEBUG_ENABLED Then HttpContext.Current.Response.Write(LinkCategory_edit.DebugMsg) %>
...
<% If LinkCategory.ID.Visible Then ' ID %>
	<tr<%= LinkCategory.RowAttributes %>>
		...
		<td<%= LinkCategory.ID.CellAttributes %>><span id="el_ID">
<div<%= LinkCategory.ID.ViewAttributes %>><%= LinkCategory.ID.EditValue %></div>
<input type="hidden" name="x_ID" id="x_ID" value="<%= ew_HTMLEncode(LinkCategory.ID.CurrentValue) %>" />
</span><%= LinkCategory.ID.CustomMsg %></td>
	</tr>
<% End If %>
