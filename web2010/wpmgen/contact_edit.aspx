<%@ Page ClassName="contact_edit" Language="VB" MasterPageFile="masterpage.master" ValidateRequest="false" AutoEventWireup="false" CodeFile="contact_edit.aspx.vb" Inherits="contact_edit" CodeFileBaseClass="AspNetMaker8_wpmWebsite" %>
...
Contact_edit.FormID = "fContactedit"; // form ID 
var EW_PAGE_ID = Contact_edit.PageID; // for backward compatibility
// extend page with ValidateForm function
Contact_edit.ValidateForm = function(fobj) {
...
Contact_edit.Form_CustomValidate =  
 function(fobj) { // DO NOT CHANGE THIS LINE!
 	// Your custom validation code here, return false if invalid. 
 	return true;
 }
<% If EW_CLIENT_VALIDATE Then %>
Contact_edit.ValidateRequired = true; // uses JavaScript validation
<% Else %>
Contact_edit.ValidateRequired = false; // no JavaScript validation
<% End If %>
//-->
</script>
<script type="text/javascript">
...
<p><span class="aspnetmaker"><%= Language.Phrase("Edit") %>&nbsp;<%= Language.Phrase("TblTypeTABLE") %><%= Contact.TableCaption %><br /><br />
<a href="<%= Contact.ReturnUrl %>"><%= Language.Phrase("GoBack") %></a></span></p>
<% If EW_DEBUG_ENABLED Then HttpContext.Current.Response.Write(Contact_edit.DebugMsg) %>
<% Contact_edit.ShowMessage() %>
<form name="fContactedit" id="fContactedit" method="post" onsubmit="this.action=location.pathname;return Contact_edit.ValidateForm(this);">
<p />
<input type="hidden" name="a_table" id="a_table" value="Contact" />
...
<input type="hidden" name="x_ContactID" id="x_ContactID" value="<%= ew_HTMLEncode(Contact.ContactID.CurrentValue) %>" />
<p />
