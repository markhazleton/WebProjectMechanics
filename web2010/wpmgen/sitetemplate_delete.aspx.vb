...
Partial Class sitetemplate_delete
	...
	Class cSiteTemplate_delete
		...
		' ASP.NET page object
		Public ReadOnly Property AspNetPage() As sitetemplate_delete
			Get
				Return CType(m_ParentPage, sitetemplate_delete)
			End Get
		End Property
	...
	Sub Page_Main()
		...
		For i As Integer = 0 to arRecKeys.GetUpperBound(0)
			...
			' Set up key field
			sKeyFld = sKey
			sFilter &= "[TemplatePrefix]='" & ew_AdjustSql(sKeyFld) & "' AND "
    ...
    user = CurrentUserName()
		...
		' Name Field
		ew_WriteAuditTrail(filePfx, curDateTime, id, user, action, table, "Name", keyvalue, RsSrc("Name"), newvalue)

		' CSS Field
		ew_WriteAuditTrail(filePfx, curDateTime, id, user, action, table, "CSS", keyvalue, "<MEMO>", newvalue)

		' Top Field
		ew_WriteAuditTrail(filePfx, curDateTime, id, user, action, table, "Top", keyvalue, "<MEMO>", newvalue)

		' Bottom Field
		ew_WriteAuditTrail(filePfx, curDateTime, id, user, action, table, "Bottom", keyvalue, "<MEMO>", newvalue)
	End Sub
