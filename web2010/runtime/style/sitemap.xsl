...
<xsl:stylesheet version="2.0" 
  ...
  <xsl:template match="/">
    <html>     
      ...
      <body onLoad="initXsl('table0','{$fileType}');">  
        ...
        <!-- Text and table -->
        <h1 id="head1">WPM XML Sitemap (ProjectMechanics.com) </h1>        
        <xsl:choose>
  ...
  <xsl:template name="siteindexTable">
    <h2>Number of URLs in this xml sitemap: <xsl:value-of select="count(urlset/url)"></xsl:value-of></h2>          
    <p class="sml">Click on the table headers to change sorting.</p>
      <p class="sml">
          <xsl:for-each select="url">1</xsl:for-each>
      </p>
