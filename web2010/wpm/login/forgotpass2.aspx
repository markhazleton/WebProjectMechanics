...
<script language="vbscript" runat="Server">
    ...
    Dim mySiteMap As New wpmActiveSite(httpcontext.Current.Session)
</script>
<%
    ...
    FF_RecConn.Open(wpmSession.GetSiteDB & ";")
    strSQL = "SELECT Contact.ContactID, " & "Contact.LogonName, " & "Contact.PrimaryContact, " & "Contact.GroupID, " & "Contact.LogonPassword, Contact.active, " & "Contact.Email, " & "Company.CompanyID, " & "Company.CompanyName, " & "Contact.TemplatePrefix " & "FROM Contact,Company " & "WHERE Contact.CompanyID=Company.CompanyID " & "and Contact.CompanyID=" & wpmSession.GetCompanyID() & " " & "and Contact.Email = '" & Replace(rsPassword__MMColParam, "'", "''") & "' "
    With rsPassword
    ...
    With RSBODY
        .let_ActiveConnection(FF_RecConn)
        .let_Source("SELECT * FROM Company where CompanyID=" & wpmSession.GetCompanyID() & " ")
        .CursorType = 0
        .CursorLocation = 2
        .LockType = 1
        .Open()
    ...
    Else
        ...
        mailSubject = "Your User Name and Password for " & mySiteMap.SiteURL
        mailBody = "Your user name and password for " & mySiteMap.SiteURL & " as requested:" & vbCrLf & "User Name : " & (IIf(IsDBNull(rsPassword.Fields.Item("LogonName").Value), Nothing, rsPassword.Fields.Item("LogonName").Value)) & vbCrLf & "Password  : " & (IIf(IsDBNull(rsPassword.Fields.Item("LogonPassword").Value), Nothing, rsPassword.Fields.Item("LogonPassword").Value)) & vbCrLf & vbCrLf & "You may now use these to log on to " & mySiteMap.SiteURL & "."
