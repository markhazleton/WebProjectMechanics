...
Namespace PMGEN
	...
	''' <summary>
	''' Summary description for SiteCategoryGroupbll
	''' Bussiness Layer (SiteCategoryGroup)
	''' </summary>

	Public MustInherit Class SiteCategoryGroupbll_base
		Public Sub New() 
		End Sub
		...
		Public Shared Function Inserting(ByVal row1 As SiteCategoryGrouprow) As Boolean
			Return True
		End Function
		...
		Public Shared Sub Inserted(ByVal row1 As SiteCategoryGrouprow)
		...
		Public Shared Function Updating(ByVal row1 As SiteCategoryGrouprow, ByVal row2 As SiteCategoryGrouprow) As Boolean
			Return True
		End Function
		...
		Public Shared Sub Updated(ByVal row1 As SiteCategoryGrouprow, ByVal row2 As SiteCategoryGrouprow)
		...
		Public Shared Function Deleting(ByVal rows As Collection(Of SiteCategoryGrouprow)) As Boolean
			Return True
		End Function
		...
		Public Shared Sub Deleted(ByVal rows As Collection(Of SiteCategoryGrouprow))
			Dim i As Integer = 0
			Do While (i < rows.Count)
				WriteAuditTrail(DirectCast(rows(i), SiteCategoryGrouprow), Nothing, Core.PageType.Delete)
				i += 1
			Loop
		End Sub
		...
		Private Shared Sub WriteAuditTrail(ByVal row1 As SiteCategoryGrouprow, ByVal row2 As SiteCategoryGrouprow, ByVal pt As Core.PageType)
			...
			Dim strTable As String = "SiteCategoryGroup"
			...
			If (row1 IsNot Nothing) Then strKeys &= row1.SiteCategoryGroupID.ToString() Else strKeys &= row2.SiteCategoryGroupID.ToString()
