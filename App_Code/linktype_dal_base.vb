...
Imports EW.Web
Namespace PMGEN
	Public MustInherit class LinkTypedal_base
		...
		Dim objAdvancedSearchParm1 As LinkTyperow ' Advanced Search Parm 1
		Dim objAdvancedSearchParm2 As LinkTyperow ' Advanced Search Parm 2
		...
		Public Sub New (ByVal isUseSession As Boolean)
			...
			If (bUseSession) Then
				...
				' Advanced Search arguments 
				Dim sSearchName As String = Share.ProjectName + "_" + LinkTypeinf.TableVar + "_Search1" 
				objAdvancedSearchParm1 = LoadSearch(sSearchName) 
				sSearchName = Share.ProjectName + "_" + LinkTypeinf.TableVar + "_Search2" 
				objAdvancedSearchParm2 = LoadSearch(sSearchName) 
			Else
				objAdvancedSearchParm1 = New LinkTyperow()
				objAdvancedSearchParm2 = New LinkTyperow()
			End If
		End Sub
		...
		Public ReadOnly Property AdvancedSearchParm1 As LinkTyperow 
			Get 
		...
		Public Readonly Property AdvancedSearchParm2 As LinkTyperow 
			Get 
		...
		Private Sub SaveProperties()
			If (bUseSession) Then 
				...
				Dim sSearchName As String = String.Concat(Share.ProjectName, "_", LinkTypeinf.TableVar, "_Search1") 
				SaveSearch(sSearchName, objAdvancedSearchParm1) 
				sSearchName = String.Concat(Share.ProjectName, "_", LinkTypeinf.TableVar, "_Search2") 
		...
		Private Function LoadSearch(ByVal objectName As String) As LinkTyperow 
			If (bUseSession) Then
				If (HttpContext.Current.Session(objectName) Is Nothing) Then
					HttpContext.Current.Session(objectName) = New LinkTyperow()
				End If
				Return TryCast(HttpContext.Current.Session(objectName), LinkTyperow)
			Else
				Return Nothing
			End If
		End Function
		...
		Private Sub SaveSearch(ByVal objectName As String, ByVal row As LinkTyperow)
			If (bUseSession) Then
				If (HttpContext.Current.Session(objectName) Is Nothing) Then
					HttpContext.Current.Session(objectName) = New LinkTyperow()
				End If
		...
		Private Function GetAdvancedSearch(ByVal row1 As LinkTyperow, ByVal row2 As LinkTyperow, ByRef parameters As DbParameterCollection ) As String 
			...
			If (bUseSession) Then
				...
				' LinkTypeDesc Field 
				parm = Core.SetAdvancedSearchParm(row1.Attribute("LinkTypeDesc").SearchType)
				strParmWhere.Remove(0, strParmWhere.Length)
				If (parm.Operator.Length > 0) Then
					strParmWhere.Append(String.Concat("([LinkTypeDesc] ", parm.Operator, " ",  Database.GetSqlParameterName(Db.DbType, "AdvSearch1__LinkTypeDesc"), ")"))
					oParameter = New OleDbParameter("AdvSearch1__LinkTypeDesc", OleDbType.VarWChar)
					oParameter.Value = parm.Prefix + row1.LinkTypeDesc + parm.Suffix
					parameters.Add(oParameter)
				End If
				parm = Core.SetAdvancedSearchParm(row2.Attribute("LinkTypeDesc").SearchType)
				If (parm.Operator.Length > 0) Then
					If (strParmWhere.Length > 0) Then strParmWhere.Append(String.Concat(" ", row1.Attribute("LinkTypeDesc").SearchCond.ToString(), " "))
					strParmWhere.Append(String.Concat("([LinkTypeDesc] ", parm.Operator, " ",  Database.GetSqlParameterName(Db.DbType, "AdvSearch2__LinkTypeDesc"), ")"))
					oParameter = New OleDbParameter("AdvSearch2__LinkTypeDesc", OleDbType.VarWChar)
					oParameter.Value = parm.Prefix + row2.LinkTypeDesc + parm.Suffix
					parameters.Add(oParameter)
				End If
				If (strParmWhere.Length > 0) Then 
					If (strWhere.Length > 0) Then strWhere.Append(" AND ")
				...
				parm = Core.SetAdvancedSearchParm(row1.Attribute("LinkTypeComment").SearchType)
				strParmWhere.Remove(0, strParmWhere.Length)
				If (parm.Operator.Length > 0) Then
					strParmWhere.Append(String.Concat("([LinkTypeComment] ", parm.Operator, " ",  Database.GetSqlParameterName(Db.DbType, "AdvSearch1__LinkTypeComment"), ")"))
					oParameter = New OleDbParameter("AdvSearch1__LinkTypeComment", OleDbType.LongVarWChar)
					oParameter.Value = parm.Prefix + row1.LinkTypeComment + parm.Suffix
					parameters.Add(oParameter)
				End If
				parm = Core.SetAdvancedSearchParm(row2.Attribute("LinkTypeComment").SearchType)
				If (parm.Operator.Length > 0) Then
					If (strParmWhere.Length > 0) Then strParmWhere.Append(String.Concat(" ", row1.Attribute("LinkTypeComment").SearchCond.ToString(), " "))
					strParmWhere.Append(String.Concat("([LinkTypeComment] ", parm.Operator, " ",  Database.GetSqlParameterName(Db.DbType, "AdvSearch2__LinkTypeComment"), ")"))
					oParameter = New OleDbParameter("AdvSearch2__LinkTypeComment", OleDbType.LongVarWChar)
					oParameter.Value = parm.Prefix + row2.LinkTypeComment + parm.Suffix
					parameters.Add(oParameter)
				End If
				If (strParmWhere.Length > 0) Then 
					If (strWhere.Length > 0) Then strWhere.Append(" AND ")
				...
				parm = Core.SetAdvancedSearchParm(row1.Attribute("LinkTypeTarget").SearchType)
				strParmWhere.Remove(0, strParmWhere.Length)
				If (parm.Operator.Length > 0) Then
					strParmWhere.Append(String.Concat("([LinkTypeTarget] ", parm.Operator, " ",  Database.GetSqlParameterName(Db.DbType, "AdvSearch1__LinkTypeTarget"), ")"))
					oParameter = New OleDbParameter("AdvSearch1__LinkTypeTarget", OleDbType.VarWChar)
					oParameter.Value = parm.Prefix + row1.LinkTypeTarget + parm.Suffix
					parameters.Add(oParameter)
				End If
				parm = Core.SetAdvancedSearchParm(row2.Attribute("LinkTypeTarget").SearchType)
				If (parm.Operator.Length > 0) Then
					If (strParmWhere.Length > 0) Then strParmWhere.Append(String.Concat(" ", row1.Attribute("LinkTypeTarget").SearchCond.ToString(), " "))
					strParmWhere.Append(String.Concat("([LinkTypeTarget] ", parm.Operator, " ",  Database.GetSqlParameterName(Db.DbType, "AdvSearch2__LinkTypeTarget"), ")"))
					oParameter = New OleDbParameter("AdvSearch2__LinkTypeTarget", OleDbType.VarWChar)
					oParameter.Value = parm.Prefix + row2.LinkTypeTarget + parm.Suffix
					parameters.Add(oParameter)
				End If
				If (strParmWhere.Length > 0) Then 
					If (strWhere.Length > 0) Then strWhere.Append(" AND ")
		...
		Private Function BasicSearchSql(ByVal arrKeywords As String(), ByVal strOpr As String, ByRef parameters As DbParameterCollection) As String
			...
			For Each strKeyWord As String In arrKeywords
				strKeywordWhere &= (String.Concat("[LinkTypeCD] LIKE ", Database.GetSqlParameterName(Db.DbType, "BasicSearch__LinkTypeCD" & i.ToString())))
				oParameter = New OleDbParameter("BasicSearch__LinkTypeCD" & i.ToString(), OleDbType.VarWChar)
			...
			strKeywordWhere = String.Empty
			For Each strKeyWord As String In arrKeywords
				strKeywordWhere &= (String.Concat("[LinkTypeDesc] LIKE ", Database.GetSqlParameterName(Db.DbType, "BasicSearch__LinkTypeDesc" & i.ToString())))
				oParameter = New OleDbParameter("BasicSearch__LinkTypeDesc" & i.ToString(), OleDbType.VarWChar)
				oParameter.Value = "%" + strKeyword + "%"
				parameters.Add(oParameter)
				If ((strOpr.Length > 0) AndAlso (i < arrKeywords.Length - 1)) Then
					strKeywordWhere &= (String.Concat(" ", strOpr, " "))
			...
			strKeywordWhere = String.Empty
			For Each strKeyWord As String In arrKeywords
				strKeywordWhere &= (String.Concat("[LinkTypeComment] LIKE ", Database.GetSqlParameterName(Db.DbType, "BasicSearch__LinkTypeComment" & i.ToString())))
				oParameter = New OleDbParameter("BasicSearch__LinkTypeComment" & i.ToString(), OleDbType.LongVarWChar)
				oParameter.Value = "%" + strKeyword + "%"
				parameters.Add(oParameter)
				If ((strOpr.Length > 0) AndAlso (i < arrKeywords.Length - 1)) Then
					strKeywordWhere &= (String.Concat(" ", strOpr, " "))
			...
			strKeywordWhere = String.Empty
			For Each strKeyWord As String In arrKeywords
				strKeywordWhere &= (String.Concat("[LinkTypeTarget] LIKE ", Database.GetSqlParameterName(Db.DbType, "BasicSearch__LinkTypeTarget" & i.ToString())))
				oParameter = New OleDbParameter("BasicSearch__LinkTypeTarget" & i.ToString(), OleDbType.VarWChar)
				oParameter.Value = "%" + strKeyword + "%"
				parameters.Add(oParameter)
				If ((strOpr.Length > 0) AndAlso (i < arrKeywords.Length - 1)) Then
					strKeywordWhere &= (String.Concat(" ", strOpr, " "))
		...
		Public Overridable Function LoadRows(ByVal filter As String) As LinkTyperow()
			Dim rows As LinkTyperows = LoadList(filter)
			Dim array As LinkTyperow() = Nothing
			If (rows IsNot Nothing) Then
				array = New LinkTyperow(rows.Count - 1) {}
				rows.CopyTo(array, 0)
			End If
			Return array
		End Function
		...
		Private Function LoadDataViewFromSql(ByVal filter As String, ByVal orderBy As String, ByRef parameters As DbParameterCollection) As DataView 
			...
			Catch e As Exception
				...
				End If
				Throw New LinkTypeDataException(strDbErrorMessage)
			End Try
		End Function
		...
		Public Overridable Function LoadList(ByVal filter As String) As LinkTyperows 
			Dim parameters As DbParameterCollection = Db.GetParameterCollection()
			...
			Try
				...
				If (dv Is Nothing) Then
					Return Nothing
				End If
				Dim rows As LinkTyperows = New LinkTyperows()
				For Each drv As DataRowView In dv
					Dim row As LinkTyperow = GetRow(drv)
					rows.Add(row)
				Next
		...
		Public Overridable Function LoadList(ByVal filter As String, ByVal PageSize As Integer, ByVal StartRow As Integer) As LinkTyperows 
			Dim parameters As DbParameterCollection = Db.GetParameterCollection()
			Dim strWhere As String = SearchFilter(parameters) 
			Dim strMasterDetailWhere As String = MasterDetailFilter(parameters)
			If (Not String.IsNullOrEmpty(strMasterDetailWhere)) Then
		...
		Public Overridable Function LoadRow(ByVal key As LinkTypekey, ByVal filter As String) As LinkTyperow 
			Dim parameters As DbParameterCollection = Db.GetParameterCollection()
		...
		Private Function GetRow(ByVal drv As DataRowView) As LinkTyperow ' Get a row based on data reader
			...
			Try
				...
				' Field LinkTypeDesc
				If (Not DBNull.Value.Equals(drv("LinkTypeDesc"))) Then
					row.LinkTypeDesc = Convert.ToString(drv("LinkTypeDesc"))
				Else
					row.LinkTypeDesc = Nothing
				End If

				' Field LinkTypeComment
				If (Not DBNull.Value.Equals(drv("LinkTypeComment"))) Then
					row.LinkTypeComment = Convert.ToString(drv("LinkTypeComment"))
				Else
					row.LinkTypeComment = Nothing
				End If

				' Field LinkTypeTarget
				If (Not DBNull.Value.Equals(drv("LinkTypeTarget"))) Then
					row.LinkTypeTarget = Convert.ToString(drv("LinkTypeTarget"))
				Else
					row.LinkTypeTarget = Nothing
				End If
			Catch e As Exception
				Throw
			End Try
		...
		Public Overridable Sub Update(ByVal row As LinkTyperow)
			Dim oParameter As OleDbParameter = Nothing
			...
			Try
				Using oConn As DbConnection = Db.GetConnection()
					Using oCmd As DbCommand = Db.GetCommand(sSql, oConn)
						...
						End If
						If (row.Attribute("LinkTypeDesc").AllowUpdate) Then
							oParameter = New OleDbParameter("LinkTypeDesc", OleDbType.VarWChar)
							If (row.LinkTypeDesc IsNot Nothing) Then
								oParameter.Value = DirectCast(row.LinkTypeDesc, Object)
							Else
								oParameter.Value = DBNull.Value
							End If
							oCmd.Parameters.Add(oParameter) ' Update or Remove
						End If
						If (row.Attribute("LinkTypeComment").AllowUpdate) Then
							oParameter = New OleDbParameter("LinkTypeComment", OleDbType.LongVarWChar)
							If (row.LinkTypeComment IsNot Nothing) Then
								oParameter.Value = DirectCast(row.LinkTypeComment, Object)
							Else
								oParameter.Value = DBNull.Value
							End If
							oCmd.Parameters.Add(oParameter) ' Update or Remove
						End If
						If (row.Attribute("LinkTypeTarget").AllowUpdate) Then
							oParameter = New OleDbParameter("LinkTypeTarget", OleDbType.VarWChar)
							If (row.LinkTypeTarget IsNot Nothing) Then
								oParameter.Value = DirectCast(row.LinkTypeTarget, Object)
							Else
								oParameter.Value = DBNull.Value
							End If
							oCmd.Parameters.Add(oParameter) ' Update or Remove
						...
						' Key parameters
						oParameter = New OleDbParameter("LinkTypeCD", OleDbType.VarWChar)
						oParameter.Value = row.LinkTypeCD
		...
		Public Overridable Function Insert(ByVal row As LinkTyperow) As LinkTyperow 
		...
		Public Sub Delete(ByVal LinkTypeCD As String) 
			Dim oParameter As OleDbParameter = Nothing
			Dim bStatus As Boolean 
		...
		Public Sub Delete(ByVal row As LinkTyperow )
			Me.Delete(CType(row.LinkTypeCD, String))
		End Sub
		...
		Public Overridable Function KeyFilter(ByVal keys As ArrayList) As String 
			...
			Do While (i < keys.Count)
				...
				strWhere.Append(String.Concat("(", KeyFilter(DirectCast(keys(i), LinkTypekey)), ")"))
				i += 1
			Loop
			Return strWhere.ToString()
		End Function
		...
		Public Overridable Function KeyFilter(ByVal keys As ArrayList, ByRef parameters As DbParameterCollection) As String 
			...
			Do While (i < keys.Count)
				If (strWhere.Length > 0) Then strWhere.Append(" OR ") 
				strWhere.Append(String.Concat("(", KeyFilter(DirectCast(keys(i), LinkTypekey), parameters, i), ")"))
				i += 1
			Loop
			Return strWhere.ToString()
		End Function
		...
		Public Overridable Function KeyFilter(ByVal key As LinkTypekey) As String 
			Dim strWhere As StringBuilder = New StringBuilder()

			' Key parameters
			If (strWhere.Length > 0) Then strWhere.Append(" AND ")
			strWhere.Append(String.Concat("[LinkTypeCD] = '", Db.AdjustSql(key.LinkTypeCD), "'")) ' [LinkTypeCD] field
			Return strWhere.ToString()
		End Function
		...
		Public Overridable Function KeyFilter(ByVal key As LinkTypekey, ByRef parameters As DbParameterCollection) As String 
			Dim strWhere As StringBuilder = New StringBuilder()
			Dim oParameter As OleDbParameter = Nothing

			' Key parameters
			If (strWhere.Length > 0) Then strWhere.Append(" AND ")
			strWhere.Append("[LinkTypeCD] = " & Database.GetSqlParameterName(Db.DbType, "key__LinkTypeCD"))
			oParameter = New OleDbParameter("key__LinkTypeCD", OleDbType.VarWChar)
			oParameter.Value = key.LinkTypeCD
			parameters.Add(oParameter)
			Return strWhere.ToString()
		End Function
		...
		Public Overridable Function KeyFilter(ByVal key As LinkTypekey, ByRef parameters As DbParameterCollection, ByVal index As Integer) As String
			Dim strWhere As StringBuilder = New StringBuilder()
			Dim oParameter As OleDbParameter = Nothing

			' Key parameters
			If (strWhere.Length > 0) Then strWhere.Append(" AND ")
			strWhere.Append("[LinkTypeCD] = " & Database.GetSqlParameterName(Db.DbType, "key__LinkTypeCD" & index.ToString())) ' [LinkTypeCD] field
			oParameter = New OleDbParameter("key__LinkTypeCD" + index.ToString(), OleDbType.VarWChar)
			oParameter.Value = key.LinkTypeCD
			parameters.Add(oParameter)
			Return strWhere.ToString()
		End Function
		...
		Public Class LinkTypeDataException 
			Inherits Exception
