...
Imports EW.Web
Namespace PMGEN
	Public MustInherit class Linkdal_base
		...
		Dim objAdvancedSearchParm1 As Linkrow ' Advanced Search Parm 1
		Dim objAdvancedSearchParm2 As Linkrow ' Advanced Search Parm 2
		...
		Public Sub New (ByVal isUseSession As Boolean)
			...
			If (bUseSession) Then
				...
				' Advanced Search arguments 
				Dim sSearchName As String = Share.ProjectName + "_" + Linkinf.TableVar + "_Search1" 
				objAdvancedSearchParm1 = LoadSearch(sSearchName) 
				sSearchName = Share.ProjectName + "_" + Linkinf.TableVar + "_Search2" 
				objAdvancedSearchParm2 = LoadSearch(sSearchName) 
			Else
				objAdvancedSearchParm1 = New Linkrow()
				objAdvancedSearchParm2 = New Linkrow()
			End If
		End Sub
		...
		Public ReadOnly Property AdvancedSearchParm1 As Linkrow 
			Get 
		...
		Public Readonly Property AdvancedSearchParm2 As Linkrow 
			Get 
		...
		Private Sub SaveProperties()
			If (bUseSession) Then 
				...
				Dim sSearchName As String = String.Concat(Share.ProjectName, "_", Linkinf.TableVar, "_Search1") 
				SaveSearch(sSearchName, objAdvancedSearchParm1) 
				sSearchName = String.Concat(Share.ProjectName, "_", Linkinf.TableVar, "_Search2") 
		...
		Private Function LoadSearch(ByVal objectName As String) As Linkrow 
			If (bUseSession) Then
				If (HttpContext.Current.Session(objectName) Is Nothing) Then
					HttpContext.Current.Session(objectName) = New Linkrow()
				End If
				Return TryCast(HttpContext.Current.Session(objectName), Linkrow)
			Else
				Return Nothing
			End If
		End Function
		...
		Private Sub SaveSearch(ByVal objectName As String, ByVal row As Linkrow)
			If (bUseSession) Then
				If (HttpContext.Current.Session(objectName) Is Nothing) Then
					HttpContext.Current.Session(objectName) = New Linkrow()
				End If
		...
		Private Function GetAdvancedSearch(ByVal row1 As Linkrow, ByVal row2 As Linkrow, ByRef parameters As DbParameterCollection ) As String 
		...
		Private Function BasicSearchSql(ByVal arrKeywords As String(), ByVal strOpr As String, ByRef parameters As DbParameterCollection) As String
			...
			strKeywordWhere = String.Empty
			For Each strKeyWord As String In arrKeywords
				If (DataFormat.CheckInt32(strKeyword) = False) Then i += 1 : Continue For
				strKeywordWhere &= (String.Concat("[CategoryID] = ", Database.GetSqlParameterName(Db.DbType, "BasicSearch__CategoryID" & i.ToString())))
				oParameter = New OleDbParameter("BasicSearch__CategoryID" & i.ToString(), OleDbType.Integer)
				oParameter.Value = "" + strKeyword + ""
				parameters.Add(oParameter)
				If ((strOpr.Length > 0) AndAlso (i < arrKeywords.Length - 1)) Then
					strKeywordWhere &= (String.Concat(" ", strOpr, " "))
			...
			For Each strKeyWord As String In arrKeywords
				If (DataFormat.CheckInt32(strKeyword) = False) Then i += 1 : Continue For
				strKeywordWhere &= (String.Concat("[CompanyID] = ", Database.GetSqlParameterName(Db.DbType, "BasicSearch__CompanyID" & i.ToString())))
				oParameter = New OleDbParameter("BasicSearch__CompanyID" & i.ToString(), OleDbType.Integer)
				oParameter.Value = "" + strKeyword + ""
				parameters.Add(oParameter)
				If ((strOpr.Length > 0) AndAlso (i < arrKeywords.Length - 1)) Then
					strKeywordWhere &= (String.Concat(" ", strOpr, " "))
			...
			For Each strKeyWord As String In arrKeywords
				If (DataFormat.CheckInt32(strKeyword) = False) Then i += 1 : Continue For
				strKeywordWhere &= (String.Concat("[PageID] = ", Database.GetSqlParameterName(Db.DbType, "BasicSearch__PageID" & i.ToString())))
				oParameter = New OleDbParameter("BasicSearch__PageID" & i.ToString(), OleDbType.Integer)
				oParameter.Value = "" + strKeyword + ""
				parameters.Add(oParameter)
				If ((strOpr.Length > 0) AndAlso (i < arrKeywords.Length - 1)) Then
					strKeywordWhere &= (String.Concat(" ", strOpr, " "))
		...
		Public Overridable Function LoadRows(ByVal filter As String) As Linkrow()
			Dim rows As Linkrows = LoadList(filter)
			Dim array As Linkrow() = Nothing
			If (rows IsNot Nothing) Then
				array = New Linkrow(rows.Count - 1) {}
				rows.CopyTo(array, 0)
			End If
			Return array
		End Function
		...
		Private Function LoadDataViewFromSql(ByVal filter As String, ByVal orderBy As String, ByRef parameters As DbParameterCollection) As DataView 
			...
			Catch e As Exception
				...
				End If
				Throw New LinkDataException(strDbErrorMessage)
			End Try
		End Function
		...
		Public Overridable Function LoadList(ByVal filter As String) As Linkrows 
			Dim parameters As DbParameterCollection = Db.GetParameterCollection()
			...
			Try
				...
				If (dv Is Nothing) Then
					Return Nothing
				End If
				Dim rows As Linkrows = New Linkrows()
				For Each drv As DataRowView In dv
					Dim row As Linkrow = GetRow(drv)
					rows.Add(row)
				Next
		...
		Public Overridable Function LoadList(ByVal filter As String, ByVal PageSize As Integer, ByVal StartRow As Integer) As Linkrows 
			Dim parameters As DbParameterCollection = Db.GetParameterCollection()
			Dim strWhere As String = SearchFilter(parameters) 
			Dim strMasterDetailWhere As String = MasterDetailFilter(parameters)
			If (Not String.IsNullOrEmpty(strMasterDetailWhere)) Then
		...
		Public Overridable Function LoadRow(ByVal key As Linkkey, ByVal filter As String) As Linkrow 
			Dim parameters As DbParameterCollection = Db.GetParameterCollection()
		...
		Private Function GetRow(ByVal drv As DataRowView) As Linkrow ' Get a row based on data reader
		...
		Public Overridable Sub Update(ByVal row As Linkrow)
			Dim oParameter As OleDbParameter = Nothing
		...
		Public Overridable Function Insert(ByVal row As Linkrow) As Linkrow 
		...
		Public Sub Delete(ByVal row As Linkrow )
		...
		Public Overridable Function KeyFilter(ByVal keys As ArrayList) As String 
			...
			Do While (i < keys.Count)
				...
				strWhere.Append(String.Concat("(", KeyFilter(DirectCast(keys(i), Linkkey)), ")"))
				i += 1
			Loop
			Return strWhere.ToString()
		End Function
		...
		Public Overridable Function KeyFilter(ByVal keys As ArrayList, ByRef parameters As DbParameterCollection) As String 
			...
			Do While (i < keys.Count)
				If (strWhere.Length > 0) Then strWhere.Append(" OR ") 
				strWhere.Append(String.Concat("(", KeyFilter(DirectCast(keys(i), Linkkey), parameters, i), ")"))
				i += 1
			Loop
			Return strWhere.ToString()
		End Function
		...
		Public Overridable Function KeyFilter(ByVal key As Linkkey) As String 
			Dim strWhere As StringBuilder = New StringBuilder()
		...
		Public Overridable Function KeyFilter(ByVal key As Linkkey, ByRef parameters As DbParameterCollection) As String 
			Dim strWhere As StringBuilder = New StringBuilder()
			Dim oParameter As OleDbParameter = Nothing
		...
		Public Overridable Function KeyFilter(ByVal key As Linkkey, ByRef parameters As DbParameterCollection, ByVal index As Integer) As String
			Dim strWhere As StringBuilder = New StringBuilder()
			Dim oParameter As OleDbParameter = Nothing
		...
		Public Class LinkDataException 
			Inherits Exception
