...
Namespace PMGEN
	...
	''' <summary>
	''' Summary description for LinkTypebll
	''' Bussiness Layer (LinkType)
	''' </summary>

	Public MustInherit Class LinkTypebll_base
		Public Sub New() 
		End Sub
		...
		Public Shared Function Inserting(ByVal row1 As LinkTyperow) As Boolean
			Return True
		End Function
		...
		Public Shared Sub Inserted(ByVal row1 As LinkTyperow)
		...
		Public Shared Function Updating(ByVal row1 As LinkTyperow, ByVal row2 As LinkTyperow) As Boolean
			Return True
		End Function
		...
		Public Shared Sub Updated(ByVal row1 As LinkTyperow, ByVal row2 As LinkTyperow)
		...
		Public Shared Function Deleting(ByVal rows As Collection(Of LinkTyperow)) As Boolean
			Return True
		End Function
		...
		Public Shared Sub Deleted(ByVal rows As Collection(Of LinkTyperow))
			Dim i As Integer = 0
			Do While (i < rows.Count)
				WriteAuditTrail(DirectCast(rows(i), LinkTyperow), Nothing, Core.PageType.Delete)
				i += 1
			Loop
		End Sub
		...
		Private Shared Sub WriteAuditTrail(ByVal row1 As LinkTyperow, ByVal row2 As LinkTyperow, ByVal pt As Core.PageType)
			...
			Dim strTable As String = "LinkType"
			...
			Dim info As LinkTypeinf = New LinkTypeinf()
			...
			If (row1 IsNot Nothing) Then strKeys &= row1.LinkTypeCD.ToString() Else strKeys &= row2.LinkTypeCD.ToString()
