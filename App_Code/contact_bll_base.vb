...
Namespace PMGEN
	...
	''' <summary>
	''' Summary description for Contactbll
	''' Bussiness Layer (Contact)
	''' </summary>

	Public MustInherit Class Contactbll_base
		Public Sub New() 
		End Sub
		...
		Public Shared Function Inserting(ByVal row1 As Contactrow) As Boolean
			Return True
		End Function
		...
		Public Shared Sub Inserted(ByVal row1 As Contactrow)
		...
		Public Shared Function Updating(ByVal row1 As Contactrow, ByVal row2 As Contactrow) As Boolean
			Return True
		End Function
		...
		Public Shared Sub Updated(ByVal row1 As Contactrow, ByVal row2 As Contactrow)
		...
		Public Shared Function Deleting(ByVal rows As Collection(Of Contactrow)) As Boolean
			Return True
		End Function
		...
		Public Shared Sub Deleted(ByVal rows As Collection(Of Contactrow))
			Dim i As Integer = 0
			Do While (i < rows.Count)
				WriteAuditTrail(DirectCast(rows(i), Contactrow), Nothing, Core.PageType.Delete)
				i += 1
			Loop
		End Sub
		...
		Private Shared Sub WriteAuditTrail(ByVal row1 As Contactrow, ByVal row2 As Contactrow, ByVal pt As Core.PageType)
			...
			If (row1 IsNot Nothing) Then strKeys &= row1.ContactID.ToString() Else strKeys &= row2.ContactID.ToString()
