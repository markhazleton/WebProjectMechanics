...
Imports EW.Web
Namespace PMGEN
	Public MustInherit class SiteTemplatedal_base
		...
		Dim objAdvancedSearchParm1 As SiteTemplaterow ' Advanced Search Parm 1
		Dim objAdvancedSearchParm2 As SiteTemplaterow ' Advanced Search Parm 2
		...
		Public Sub New (ByVal isUseSession As Boolean)
			...
			If (bUseSession) Then
				...
				' Advanced Search arguments 
				Dim sSearchName As String = Share.ProjectName + "_" + SiteTemplateinf.TableVar + "_Search1" 
				objAdvancedSearchParm1 = LoadSearch(sSearchName) 
				sSearchName = Share.ProjectName + "_" + SiteTemplateinf.TableVar + "_Search2" 
				objAdvancedSearchParm2 = LoadSearch(sSearchName) 
			Else
				objAdvancedSearchParm1 = New SiteTemplaterow()
				objAdvancedSearchParm2 = New SiteTemplaterow()
			End If
		End Sub
		...
		Public ReadOnly Property AdvancedSearchParm1 As SiteTemplaterow 
			Get 
		...
		Public Readonly Property AdvancedSearchParm2 As SiteTemplaterow 
			Get 
		...
		Private Sub SaveProperties()
			If (bUseSession) Then 
				...
				Dim sSearchName As String = String.Concat(Share.ProjectName, "_", SiteTemplateinf.TableVar, "_Search1") 
				SaveSearch(sSearchName, objAdvancedSearchParm1) 
				sSearchName = String.Concat(Share.ProjectName, "_", SiteTemplateinf.TableVar, "_Search2") 
		...
		Private Function LoadSearch(ByVal objectName As String) As SiteTemplaterow 
			If (bUseSession) Then
				If (HttpContext.Current.Session(objectName) Is Nothing) Then
					HttpContext.Current.Session(objectName) = New SiteTemplaterow()
				End If
				Return TryCast(HttpContext.Current.Session(objectName), SiteTemplaterow)
			Else
				Return Nothing
			End If
		End Function
		...
		Private Sub SaveSearch(ByVal objectName As String, ByVal row As SiteTemplaterow)
			If (bUseSession) Then
				If (HttpContext.Current.Session(objectName) Is Nothing) Then
					HttpContext.Current.Session(objectName) = New SiteTemplaterow()
				End If
		...
		Private Function GetAdvancedSearch(ByVal row1 As SiteTemplaterow, ByVal row2 As SiteTemplaterow, ByRef parameters As DbParameterCollection ) As String 
		...
		Public Overridable Function LoadRows(ByVal filter As String) As SiteTemplaterow()
			Dim rows As SiteTemplaterows = LoadList(filter)
			Dim array As SiteTemplaterow() = Nothing
			If (rows IsNot Nothing) Then
				array = New SiteTemplaterow(rows.Count - 1) {}
				rows.CopyTo(array, 0)
			End If
			Return array
		End Function
		...
		Private Function LoadDataViewFromSql(ByVal filter As String, ByVal orderBy As String, ByRef parameters As DbParameterCollection) As DataView 
			...
			Catch e As Exception
				...
				End If
				Throw New SiteTemplateDataException(strDbErrorMessage)
			End Try
		End Function
		...
		Public Overridable Function LoadList(ByVal filter As String) As SiteTemplaterows 
			Dim parameters As DbParameterCollection = Db.GetParameterCollection()
			...
			Try
				...
				If (dv Is Nothing) Then
					Return Nothing
				End If
				Dim rows As SiteTemplaterows = New SiteTemplaterows()
				For Each drv As DataRowView In dv
					Dim row As SiteTemplaterow = GetRow(drv)
					rows.Add(row)
				Next
		...
		Public Overridable Function LoadList(ByVal filter As String, ByVal PageSize As Integer, ByVal StartRow As Integer) As SiteTemplaterows 
			Dim parameters As DbParameterCollection = Db.GetParameterCollection()
			Dim strWhere As String = SearchFilter(parameters) 
			Dim strMasterDetailWhere As String = MasterDetailFilter(parameters)
			If (Not String.IsNullOrEmpty(strMasterDetailWhere)) Then
		...
		Public Overridable Function LoadRow(ByVal key As SiteTemplatekey, ByVal filter As String) As SiteTemplaterow 
			Dim parameters As DbParameterCollection = Db.GetParameterCollection()
		...
		Private Function GetRow(ByVal drv As DataRowView) As SiteTemplaterow ' Get a row based on data reader
			...
			Try
				...
				' Field Name
				If (Not DBNull.Value.Equals(drv("Name"))) Then
					row.Name = Convert.ToString(drv("Name"))
				Else
					row.Name = Nothing
				End If

				' Field Top
				If (Not DBNull.Value.Equals(drv("Top"))) Then
					row.Top = Convert.ToString(drv("Top"))
				Else
					row.Top = Nothing
				End If

				' Field Bottom
				If (Not DBNull.Value.Equals(drv("Bottom"))) Then
					row.Bottom = Convert.ToString(drv("Bottom"))
				Else
					row.Bottom = Nothing
				End If

				' Field CSS
				If (Not DBNull.Value.Equals(drv("CSS"))) Then
					row.CSS = Convert.ToString(drv("CSS"))
				Else
					row.CSS = Nothing
				End If
			Catch e As Exception
		...
		Public Overridable Sub Update(ByVal row As SiteTemplaterow)
			Dim oParameter As OleDbParameter = Nothing
			...
			Try
				Using oConn As DbConnection = Db.GetConnection()
					Using oCmd As DbCommand = Db.GetCommand(sSql, oConn)
						...
						End If
						If (row.Attribute("Name").AllowUpdate) Then
							oParameter = New OleDbParameter("Name", OleDbType.VarWChar)
							If (Not String.IsNullOrEmpty(row.Name)) Then
								oParameter.Value = DirectCast(row.Name, Object)
							Else
								oParameter.Value = DBNull.Value
							End If
							oCmd.Parameters.Add(oParameter) ' Update or Remove
						End If
						If (row.Attribute("Top").AllowUpdate) Then
							oParameter = New OleDbParameter("Top", OleDbType.LongVarWChar)
							If (row.Top IsNot Nothing) Then
								oParameter.Value = DirectCast(row.Top, Object)
							Else
								oParameter.Value = DBNull.Value
							End If
							oCmd.Parameters.Add(oParameter) ' Update or Remove
						End If
						If (row.Attribute("Bottom").AllowUpdate) Then
							oParameter = New OleDbParameter("Bottom", OleDbType.LongVarWChar)
							If (row.Bottom IsNot Nothing) Then
								oParameter.Value = DirectCast(row.Bottom, Object)
							Else
								oParameter.Value = DBNull.Value
							End If
							oCmd.Parameters.Add(oParameter) ' Update or Remove
						End If
						If (row.Attribute("CSS").AllowUpdate) Then
							oParameter = New OleDbParameter("CSS", OleDbType.LongVarWChar)
							If (row.CSS IsNot Nothing) Then
								oParameter.Value = DirectCast(row.CSS, Object)
							Else
								oParameter.Value = DBNull.Value
							End If
							oCmd.Parameters.Add(oParameter) ' Update or Remove
						...
						' Key parameters
						oParameter = New OleDbParameter("TemplatePrefix", OleDbType.VarWChar)
						oParameter.Value = row.TemplatePrefix
		...
		Public Overridable Function Insert(ByVal row As SiteTemplaterow) As SiteTemplaterow 
		...
		Public Sub Delete(ByVal TemplatePrefix As String) 
			Dim oParameter As OleDbParameter = Nothing
			Dim bStatus As Boolean 
			...
			Try
				Using oConn As DbConnection = Db.GetConnection()
					Using oCmd As DbCommand = Db.GetCommand(sSql, oConn)
						...
						' Key parameters
						oParameter = New OleDbParameter("TemplatePrefix", OleDbType.VarWChar)
						oParameter.Value = TemplatePrefix
						oCmd.Parameters.Add(oParameter)
						oConn.Open()
						oCmd.ExecuteNonQuery()
		...
		Public Sub Delete(ByVal row As SiteTemplaterow )
			Me.Delete(CType(row.TemplatePrefix, String))
		End Sub
		...
		Public Overridable Function KeyFilter(ByVal keys As ArrayList) As String 
			...
			Do While (i < keys.Count)
				...
				strWhere.Append(String.Concat("(", KeyFilter(DirectCast(keys(i), SiteTemplatekey)), ")"))
				i += 1
			Loop
			Return strWhere.ToString()
		End Function
		...
		Public Overridable Function KeyFilter(ByVal keys As ArrayList, ByRef parameters As DbParameterCollection) As String 
			...
			Do While (i < keys.Count)
				If (strWhere.Length > 0) Then strWhere.Append(" OR ") 
				strWhere.Append(String.Concat("(", KeyFilter(DirectCast(keys(i), SiteTemplatekey), parameters, i), ")"))
				i += 1
			Loop
			Return strWhere.ToString()
		End Function
		...
		Public Overridable Function KeyFilter(ByVal key As SiteTemplatekey) As String 
			Dim strWhere As StringBuilder = New StringBuilder()

			' Key parameters
			If (strWhere.Length > 0) Then strWhere.Append(" AND ")
			strWhere.Append(String.Concat("[TemplatePrefix] = '", Db.AdjustSql(key.TemplatePrefix), "'")) ' [TemplatePrefix] field
			Return strWhere.ToString()
		End Function
		...
		Public Overridable Function KeyFilter(ByVal key As SiteTemplatekey, ByRef parameters As DbParameterCollection) As String 
			Dim strWhere As StringBuilder = New StringBuilder()
			Dim oParameter As OleDbParameter = Nothing

			' Key parameters
			If (strWhere.Length > 0) Then strWhere.Append(" AND ")
			strWhere.Append("[TemplatePrefix] = " & Database.GetSqlParameterName(Db.DbType, "key__TemplatePrefix"))
			oParameter = New OleDbParameter("key__TemplatePrefix", OleDbType.VarWChar)
			oParameter.Value = key.TemplatePrefix
		...
		Public Overridable Function KeyFilter(ByVal key As SiteTemplatekey, ByRef parameters As DbParameterCollection, ByVal index As Integer) As String
			Dim strWhere As StringBuilder = New StringBuilder()
			Dim oParameter As OleDbParameter = Nothing

			' Key parameters
			If (strWhere.Length > 0) Then strWhere.Append(" AND ")
			strWhere.Append("[TemplatePrefix] = " & Database.GetSqlParameterName(Db.DbType, "key__TemplatePrefix" & index.ToString())) ' [TemplatePrefix] field
			oParameter = New OleDbParameter("key__TemplatePrefix" + index.ToString(), OleDbType.VarWChar)
			oParameter.Value = key.TemplatePrefix
			parameters.Add(oParameter)
			Return strWhere.ToString()
		End Function
		...
		Public Class SiteTemplateDataException 
			Inherits Exception
