...
Namespace PMGEN
	...
	''' Bussiness Layer (Group)
	''' </summary>

	Public MustInherit Class Groupbll_base
		Public Sub New() 
		End Sub
		...
		Public Shared Function Inserting(ByVal row1 As Grouprow) As Boolean
			Return True
		End Function
		...
		Public Shared Sub Inserted(ByVal row1 As Grouprow)
		...
		Public Shared Function Updating(ByVal row1 As Grouprow, ByVal row2 As Grouprow) As Boolean
			Return True
		End Function
		...
		Public Shared Sub Updated(ByVal row1 As Grouprow, ByVal row2 As Grouprow)
		...
		Public Shared Function Deleting(ByVal rows As Collection(Of Grouprow)) As Boolean
			Return True
		End Function
		...
		Public Shared Sub Deleted(ByVal rows As Collection(Of Grouprow))
			Dim i As Integer = 0
			Do While (i < rows.Count)
				WriteAuditTrail(DirectCast(rows(i), Grouprow), Nothing, Core.PageType.Delete)
				i += 1
			Loop
		End Sub
		...
		Private Shared Sub WriteAuditTrail(ByVal row1 As Grouprow, ByVal row2 As Grouprow, ByVal pt As Core.PageType)
			...
			If (row1 IsNot Nothing) Then strKeys &= row1.GroupID.ToString() Else strKeys &= row2.GroupID.ToString()
