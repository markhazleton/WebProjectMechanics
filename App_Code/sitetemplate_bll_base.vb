...
Namespace PMGEN
	...
	''' <summary>
	''' Summary description for SiteTemplatebll
	''' Bussiness Layer (SiteTemplate)
	''' </summary>

	Public MustInherit Class SiteTemplatebll_base
		Public Sub New() 
		End Sub
		...
		Public Shared Function Inserting(ByVal row1 As SiteTemplaterow) As Boolean
			Return True
		End Function
		...
		Public Shared Sub Inserted(ByVal row1 As SiteTemplaterow)
		...
		Public Shared Function Updating(ByVal row1 As SiteTemplaterow, ByVal row2 As SiteTemplaterow) As Boolean
			Return True
		End Function
		...
		Public Shared Sub Updated(ByVal row1 As SiteTemplaterow, ByVal row2 As SiteTemplaterow)
		...
		Public Shared Function Deleting(ByVal rows As Collection(Of SiteTemplaterow)) As Boolean
			Return True
		End Function
		...
		Public Shared Sub Deleted(ByVal rows As Collection(Of SiteTemplaterow))
			Dim i As Integer = 0
			Do While (i < rows.Count)
				WriteAuditTrail(DirectCast(rows(i), SiteTemplaterow), Nothing, Core.PageType.Delete)
				i += 1
			Loop
		End Sub
		...
		Private Shared Sub WriteAuditTrail(ByVal row1 As SiteTemplaterow, ByVal row2 As SiteTemplaterow, ByVal pt As Core.PageType)
			...
			Dim strTable As String = "SiteTemplate"
			...
			If (row1 IsNot Nothing) Then strKeys &= row1.TemplatePrefix.ToString() Else strKeys &= row2.TemplatePrefix.ToString()
