...
Namespace PMGEN
	...
	''' <summary>
	''' Summary description for LinkCategorybll
	''' Bussiness Layer (LinkCategory)
	''' </summary>

	Public MustInherit Class LinkCategorybll_base
		Public Sub New() 
		End Sub
		...
		Public Shared Function Inserting(ByVal row1 As LinkCategoryrow) As Boolean
			Return True
		End Function
		...
		Public Shared Sub Inserted(ByVal row1 As LinkCategoryrow)
		...
		Public Shared Function Updating(ByVal row1 As LinkCategoryrow, ByVal row2 As LinkCategoryrow) As Boolean
			Return True
		End Function
		...
		Public Shared Sub Updated(ByVal row1 As LinkCategoryrow, ByVal row2 As LinkCategoryrow)
		...
		Public Shared Function Deleting(ByVal rows As Collection(Of LinkCategoryrow)) As Boolean
			Return True
		End Function
		...
		Public Shared Sub Deleted(ByVal rows As Collection(Of LinkCategoryrow))
			Dim i As Integer = 0
			Do While (i < rows.Count)
				WriteAuditTrail(DirectCast(rows(i), LinkCategoryrow), Nothing, Core.PageType.Delete)
				i += 1
			Loop
		End Sub
		...
		Private Shared Sub WriteAuditTrail(ByVal row1 As LinkCategoryrow, ByVal row2 As LinkCategoryrow, ByVal pt As Core.PageType)
			...
			Dim strTable As String = "LinkCategory"
			...
			If (row1 IsNot Nothing) Then strKeys &= row1.ID.ToString() Else strKeys &= row2.ID.ToString()
