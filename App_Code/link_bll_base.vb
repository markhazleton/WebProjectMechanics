...
Namespace PMGEN
	...
	''' <summary>
	''' Summary description for Linkbll
	''' Bussiness Layer (Link)
	''' </summary>

	Public MustInherit Class Linkbll_base
		Public Sub New() 
		End Sub
		...
		Public Shared Function Inserting(ByVal row1 As Linkrow) As Boolean
			Return True
		End Function
		...
		Public Shared Sub Inserted(ByVal row1 As Linkrow)
		...
		Public Shared Function Updating(ByVal row1 As Linkrow, ByVal row2 As Linkrow) As Boolean
			Return True
		End Function
		...
		Public Shared Sub Updated(ByVal row1 As Linkrow, ByVal row2 As Linkrow)
		...
		Public Shared Function Deleting(ByVal rows As Collection(Of Linkrow)) As Boolean
			Return True
		End Function
		...
		Public Shared Sub Deleted(ByVal rows As Collection(Of Linkrow))
			Dim i As Integer = 0
			Do While (i < rows.Count)
				WriteAuditTrail(DirectCast(rows(i), Linkrow), Nothing, Core.PageType.Delete)
				i += 1
			Loop
		End Sub
		...
		Private Shared Sub WriteAuditTrail(ByVal row1 As Linkrow, ByVal row2 As Linkrow, ByVal pt As Core.PageType)
