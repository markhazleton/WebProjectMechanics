...
Namespace PMGEN
	...
	''' <summary>
	''' Summary description for SiteCategorybll
	''' Bussiness Layer (SiteCategory)
	''' </summary>

	Public MustInherit Class SiteCategorybll_base
		Public Sub New() 
		End Sub
		...
		Public Shared Function Inserting(ByVal row1 As SiteCategoryrow) As Boolean
			Return True
		End Function
		...
		Public Shared Sub Inserted(ByVal row1 As SiteCategoryrow)
		...
		Public Shared Function Updating(ByVal row1 As SiteCategoryrow, ByVal row2 As SiteCategoryrow) As Boolean
			Return True
		End Function
		...
		Public Shared Sub Updated(ByVal row1 As SiteCategoryrow, ByVal row2 As SiteCategoryrow)
		...
		Public Shared Function Deleting(ByVal rows As Collection(Of SiteCategoryrow)) As Boolean
			Return True
		End Function
		...
		Public Shared Sub Deleted(ByVal rows As Collection(Of SiteCategoryrow))
			Dim i As Integer = 0
			Do While (i < rows.Count)
				WriteAuditTrail(DirectCast(rows(i), SiteCategoryrow), Nothing, Core.PageType.Delete)
				i += 1
			Loop
		End Sub
		...
		Private Shared Sub WriteAuditTrail(ByVal row1 As SiteCategoryrow, ByVal row2 As SiteCategoryrow, ByVal pt As Core.PageType)
			...
			Dim strTable As String = "SiteCategory"
			...
			If (row1 IsNot Nothing) Then strKeys &= row1.SiteCategoryID.ToString() Else strKeys &= row2.SiteCategoryID.ToString()
