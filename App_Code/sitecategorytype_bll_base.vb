...
Namespace PMGEN
	...
	''' <summary>
	''' Summary description for SiteCategoryTypebll
	''' Bussiness Layer (SiteCategoryType)
	''' </summary>

	Public MustInherit Class SiteCategoryTypebll_base
		Public Sub New() 
		End Sub
		...
		Public Shared Function Inserting(ByVal row1 As SiteCategoryTyperow) As Boolean
			Return True
		End Function
		...
		Public Shared Sub Inserted(ByVal row1 As SiteCategoryTyperow)
		...
		Public Shared Function Updating(ByVal row1 As SiteCategoryTyperow, ByVal row2 As SiteCategoryTyperow) As Boolean
			Return True
		End Function
		...
		Public Shared Sub Updated(ByVal row1 As SiteCategoryTyperow, ByVal row2 As SiteCategoryTyperow)
		...
		Public Shared Function Deleting(ByVal rows As Collection(Of SiteCategoryTyperow)) As Boolean
			Return True
		End Function
		...
		Public Shared Sub Deleted(ByVal rows As Collection(Of SiteCategoryTyperow))
			Dim i As Integer = 0
			Do While (i < rows.Count)
				WriteAuditTrail(DirectCast(rows(i), SiteCategoryTyperow), Nothing, Core.PageType.Delete)
				i += 1
			Loop
		End Sub
		...
		Private Shared Sub WriteAuditTrail(ByVal row1 As SiteCategoryTyperow, ByVal row2 As SiteCategoryTyperow, ByVal pt As Core.PageType)
			...
			Dim strTable As String = "SiteCategoryType"
			...
			If (row1 IsNot Nothing) Then strKeys &= row1.SiteCategoryTypeID.ToString() Else strKeys &= row2.SiteCategoryTypeID.ToString()
