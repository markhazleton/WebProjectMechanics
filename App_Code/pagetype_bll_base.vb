...
Namespace PMGEN
	...
	''' <summary>
	''' Summary description for PageTypebll
	''' Bussiness Layer (PageType)
	''' </summary>

	Public MustInherit Class PageTypebll_base
		Public Sub New() 
		End Sub
		...
		Public Shared Function Inserting(ByVal row1 As PageTyperow) As Boolean
			Return True
		End Function
		...
		Public Shared Sub Inserted(ByVal row1 As PageTyperow)
		...
		Public Shared Function Updating(ByVal row1 As PageTyperow, ByVal row2 As PageTyperow) As Boolean
			Return True
		End Function
		...
		Public Shared Sub Updated(ByVal row1 As PageTyperow, ByVal row2 As PageTyperow)
		...
		Public Shared Function Deleting(ByVal rows As Collection(Of PageTyperow)) As Boolean
			Return True
		End Function
		...
		Public Shared Sub Deleted(ByVal rows As Collection(Of PageTyperow))
			Dim i As Integer = 0
			Do While (i < rows.Count)
				WriteAuditTrail(DirectCast(rows(i), PageTyperow), Nothing, Core.PageType.Delete)
				i += 1
			Loop
		End Sub
		...
		Private Shared Sub WriteAuditTrail(ByVal row1 As PageTyperow, ByVal row2 As PageTyperow, ByVal pt As Core.PageType)
			...
			Dim strTable As String = "PageType"
			...
			Dim info As PageTypeinf = New PageTypeinf()
			...
			If (row1 IsNot Nothing) Then strKeys &= row1.PageTypeID.ToString() Else strKeys &= row2.PageTypeID.ToString()
