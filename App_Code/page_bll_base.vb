...
Namespace PMGEN
	...
	''' <summary>
	''' Summary description for Pagebll
	''' Bussiness Layer (Page)
	''' </summary>

	Public MustInherit Class Pagebll_base
		Public Sub New() 
		End Sub
		...
		Public Shared Function Inserting(ByVal row1 As Pagerow) As Boolean
			Return True
		End Function
		...
		Public Shared Sub Inserted(ByVal row1 As Pagerow)
		...
		Public Shared Function Updating(ByVal row1 As Pagerow, ByVal row2 As Pagerow) As Boolean
			Return True
		End Function
		...
		Public Shared Sub Updated(ByVal row1 As Pagerow, ByVal row2 As Pagerow)
		...
		Public Shared Function Deleting(ByVal rows As Collection(Of Pagerow)) As Boolean
			Return True
		End Function
		...
		Public Shared Sub Deleted(ByVal rows As Collection(Of Pagerow))
			Dim i As Integer = 0
			Do While (i < rows.Count)
				WriteAuditTrail(DirectCast(rows(i), Pagerow), Nothing, Core.PageType.Delete)
				i += 1
			Loop
		End Sub
		...
		Private Shared Sub WriteAuditTrail(ByVal row1 As Pagerow, ByVal row2 As Pagerow, ByVal pt As Core.PageType)
			...
			If (row1 IsNot Nothing) Then strKeys &= row1.PageID.ToString() Else strKeys &= row2.PageID.ToString()
