...
Namespace PMGEN
	...
	''' <summary>
	''' Summary description for Imagebll
	''' Bussiness Layer (Image)
	''' </summary>

	Public MustInherit Class Imagebll_base
		Public Sub New() 
		End Sub
		...
		Public Shared Function Inserting(ByVal row1 As Imagerow) As Boolean
			Return True
		End Function
		...
		Public Shared Sub Inserted(ByVal row1 As Imagerow)
		...
		Public Shared Function Updating(ByVal row1 As Imagerow, ByVal row2 As Imagerow) As Boolean
			Return True
		End Function
		...
		Public Shared Sub Updated(ByVal row1 As Imagerow, ByVal row2 As Imagerow)
		...
		Public Shared Function Deleting(ByVal rows As Collection(Of Imagerow)) As Boolean
			Return True
		End Function
		...
		Public Shared Sub Deleted(ByVal rows As Collection(Of Imagerow))
			Dim i As Integer = 0
			Do While (i < rows.Count)
				WriteAuditTrail(DirectCast(rows(i), Imagerow), Nothing, Core.PageType.Delete)
				i += 1
			Loop
		End Sub
		...
		Private Shared Sub WriteAuditTrail(ByVal row1 As Imagerow, ByVal row2 As Imagerow, ByVal pt As Core.PageType)
			...
			If (row1 IsNot Nothing) Then strKeys &= row1.ImageID.ToString() Else strKeys &= row2.ImageID.ToString()
