...
Imports EW.Web
Namespace PMGEN
	Public MustInherit class Groupdal_base
		...
		Dim objAdvancedSearchParm1 As Grouprow ' Advanced Search Parm 1
		Dim objAdvancedSearchParm2 As Grouprow ' Advanced Search Parm 2
		...
		Public Sub New (ByVal isUseSession As Boolean)
			...
			If (bUseSession) Then
				...
				' Advanced Search arguments 
				Dim sSearchName As String = Share.ProjectName + "_" + Groupinf.TableVar + "_Search1" 
				objAdvancedSearchParm1 = LoadSearch(sSearchName) 
				sSearchName = Share.ProjectName + "_" + Groupinf.TableVar + "_Search2" 
				objAdvancedSearchParm2 = LoadSearch(sSearchName) 
			Else
				objAdvancedSearchParm1 = New Grouprow()
				objAdvancedSearchParm2 = New Grouprow()
			End If
		End Sub
		...
		Public ReadOnly Property AdvancedSearchParm1 As Grouprow 
			Get 
		...
		Public Readonly Property AdvancedSearchParm2 As Grouprow 
			Get 
		...
		Private Sub SaveProperties()
			If (bUseSession) Then 
				...
				Dim sSearchName As String = String.Concat(Share.ProjectName, "_", Groupinf.TableVar, "_Search1") 
				SaveSearch(sSearchName, objAdvancedSearchParm1) 
				sSearchName = String.Concat(Share.ProjectName, "_", Groupinf.TableVar, "_Search2") 
		...
		Private Function LoadSearch(ByVal objectName As String) As Grouprow 
			If (bUseSession) Then
				If (HttpContext.Current.Session(objectName) Is Nothing) Then
					HttpContext.Current.Session(objectName) = New Grouprow()
				End If
				Return TryCast(HttpContext.Current.Session(objectName), Grouprow)
			Else
				Return Nothing
			End If
		End Function
		...
		Private Sub SaveSearch(ByVal objectName As String, ByVal row As Grouprow)
			If (bUseSession) Then
				If (HttpContext.Current.Session(objectName) Is Nothing) Then
					HttpContext.Current.Session(objectName) = New Grouprow()
				End If
		...
		Private Function GetAdvancedSearch(ByVal row1 As Grouprow, ByVal row2 As Grouprow, ByRef parameters As DbParameterCollection ) As String 
		...
		Private Function BasicSearchSql(ByVal arrKeywords As String(), ByVal strOpr As String, ByRef parameters As DbParameterCollection) As String
			...
			For Each strKeyWord As String In arrKeywords
				strKeywordWhere &= (String.Concat("[GroupName] LIKE ", Database.GetSqlParameterName(Db.DbType, "BasicSearch__GroupName" & i.ToString())))
				oParameter = New OleDbParameter("BasicSearch__GroupName" & i.ToString(), OleDbType.VarWChar)
			...
			strKeywordWhere = String.Empty
			For Each strKeyWord As String In arrKeywords
				strKeywordWhere &= (String.Concat("[GroupComment] LIKE ", Database.GetSqlParameterName(Db.DbType, "BasicSearch__GroupComment" & i.ToString())))
				oParameter = New OleDbParameter("BasicSearch__GroupComment" & i.ToString(), OleDbType.VarWChar)
				oParameter.Value = "%" + strKeyword + "%"
				parameters.Add(oParameter)
				If ((strOpr.Length > 0) AndAlso (i < arrKeywords.Length - 1)) Then
					strKeywordWhere &= (String.Concat(" ", strOpr, " "))
		...
		Public Overridable Function LoadRows(ByVal filter As String) As Grouprow()
			Dim rows As Grouprows = LoadList(filter)
			Dim array As Grouprow() = Nothing
			If (rows IsNot Nothing) Then
				array = New Grouprow(rows.Count - 1) {}
				rows.CopyTo(array, 0)
			End If
			Return array
		End Function
		...
		Private Function LoadDataViewFromSql(ByVal filter As String, ByVal orderBy As String, ByRef parameters As DbParameterCollection) As DataView 
			...
			Catch e As Exception
				...
				End If
				Throw New GroupDataException(strDbErrorMessage)
			End Try
		End Function
		...
		Public Overridable Function LoadList(ByVal filter As String) As Grouprows 
			Dim parameters As DbParameterCollection = Db.GetParameterCollection()
			...
			Try
				...
				If (dv Is Nothing) Then
					Return Nothing
				End If
				Dim rows As Grouprows = New Grouprows()
				For Each drv As DataRowView In dv
					Dim row As Grouprow = GetRow(drv)
					rows.Add(row)
				Next
		...
		Public Overridable Function LoadList(ByVal filter As String, ByVal PageSize As Integer, ByVal StartRow As Integer) As Grouprows 
			Dim parameters As DbParameterCollection = Db.GetParameterCollection()
			Dim strWhere As String = SearchFilter(parameters) 
			Dim strMasterDetailWhere As String = MasterDetailFilter(parameters)
			If (Not String.IsNullOrEmpty(strMasterDetailWhere)) Then
		...
		Public Overridable Function LoadRow(ByVal key As Groupkey, ByVal filter As String) As Grouprow 
			Dim parameters As DbParameterCollection = Db.GetParameterCollection()
		...
		Private Function GetRow(ByVal drv As DataRowView) As Grouprow ' Get a row based on data reader
			...
			Try
				...
				' Field GroupName
				If (Not DBNull.Value.Equals(drv("GroupName"))) Then
					row.GroupName = Convert.ToString(drv("GroupName"))
				Else
					row.GroupName = Nothing
				End If

				' Field GroupComment
				If (Not DBNull.Value.Equals(drv("GroupComment"))) Then
					row.GroupComment = Convert.ToString(drv("GroupComment"))
				Else
					row.GroupComment = Nothing
				End If
			Catch e As Exception
				Throw
			End Try
		...
		Public Overridable Sub Update(ByVal row As Grouprow)
			Dim oParameter As OleDbParameter = Nothing
			...
			Try
				Using oConn As DbConnection = Db.GetConnection()
					Using oCmd As DbCommand = Db.GetCommand(sSql, oConn)
						...
						End If
						If (row.Attribute("GroupName").AllowUpdate) Then
							oParameter = New OleDbParameter("GroupName", OleDbType.VarWChar)
							If (Not String.IsNullOrEmpty(row.GroupName)) Then
								oParameter.Value = DirectCast(row.GroupName, Object)
							Else
								oParameter.Value = DBNull.Value
							End If
							oCmd.Parameters.Add(oParameter) ' Update or Remove
						End If
						If (row.Attribute("GroupComment").AllowUpdate) Then
							oParameter = New OleDbParameter("GroupComment", OleDbType.VarWChar)
							If (Not String.IsNullOrEmpty(row.GroupComment)) Then
								oParameter.Value = DirectCast(row.GroupComment, Object)
							Else
								oParameter.Value = DBNull.Value
							End If
							oCmd.Parameters.Add(oParameter) ' Update or Remove
		...
		Public Overridable Function Insert(ByVal row As Grouprow) As Grouprow 
			...
			Using oConn As DbConnection = Db.GetConnection()
				...
				Using oCmd As DbCommand = Db.GetCommand(sSql, oConn)
					Try
						...
						strDbErrorMessage = String.Empty
						bStatus = True ' Successful
						row.GroupID = Db.LastInsertedID(oConn) ' load auto-generated ID
		...
		Public Sub Delete(ByVal GroupID As Int32) 
			Dim oParameter As OleDbParameter = Nothing
			Dim bStatus As Boolean 
		...
		Public Sub Delete(ByVal row As Grouprow )
			Me.Delete(CType(row.GroupID, Int32))
		End Sub
		...
		Public Overridable Function KeyFilter(ByVal keys As ArrayList) As String 
			...
			Do While (i < keys.Count)
				...
				strWhere.Append(String.Concat("(", KeyFilter(DirectCast(keys(i), Groupkey)), ")"))
				i += 1
			Loop
			Return strWhere.ToString()
		End Function
		...
		Public Overridable Function KeyFilter(ByVal keys As ArrayList, ByRef parameters As DbParameterCollection) As String 
			...
			Do While (i < keys.Count)
				If (strWhere.Length > 0) Then strWhere.Append(" OR ") 
				strWhere.Append(String.Concat("(", KeyFilter(DirectCast(keys(i), Groupkey), parameters, i), ")"))
				i += 1
			Loop
			Return strWhere.ToString()
		End Function
		...
		Public Overridable Function KeyFilter(ByVal key As Groupkey) As String 
			Dim strWhere As StringBuilder = New StringBuilder()

			' Key parameters
			If (strWhere.Length > 0) Then strWhere.Append(" AND ")
			strWhere.Append(String.Concat("[GroupID] = ", key.GroupID)) ' [GroupID] field
			Return strWhere.ToString()
		End Function
		...
		Public Overridable Function KeyFilter(ByVal key As Groupkey, ByRef parameters As DbParameterCollection) As String 
			Dim strWhere As StringBuilder = New StringBuilder()
			Dim oParameter As OleDbParameter = Nothing

			' Key parameters
			If (strWhere.Length > 0) Then strWhere.Append(" AND ")
			strWhere.Append("[GroupID] = " & Database.GetSqlParameterName(Db.DbType, "key__GroupID"))
			oParameter = New OleDbParameter("key__GroupID", OleDbType.Integer)
			oParameter.Value = key.GroupID
			parameters.Add(oParameter)
			Return strWhere.ToString()
		End Function
		...
		Public Overridable Function KeyFilter(ByVal key As Groupkey, ByRef parameters As DbParameterCollection, ByVal index As Integer) As String
			Dim strWhere As StringBuilder = New StringBuilder()
			Dim oParameter As OleDbParameter = Nothing

			' Key parameters
			If (strWhere.Length > 0) Then strWhere.Append(" AND ")
			strWhere.Append("[GroupID] = " & Database.GetSqlParameterName(Db.DbType, "key__GroupID" & index.ToString())) ' [GroupID] field
			oParameter = New OleDbParameter("key__GroupID" + index.ToString(), OleDbType.Integer)
			oParameter.Value = key.GroupID
			parameters.Add(oParameter)
			Return strWhere.ToString()
		End Function
		...
		Public Class GroupDataException 
			Inherits Exception
