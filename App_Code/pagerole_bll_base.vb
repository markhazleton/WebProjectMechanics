...
Namespace PMGEN
	...
	''' <summary>
	''' Summary description for PageRolebll
	''' Bussiness Layer (PageRole)
	''' </summary>

	Public MustInherit Class PageRolebll_base
		Public Sub New() 
		End Sub
		...
		Public Shared Function Inserting(ByVal row1 As PageRolerow) As Boolean
			Return True
		End Function
		...
		Public Shared Sub Inserted(ByVal row1 As PageRolerow)
		...
		Public Shared Function Updating(ByVal row1 As PageRolerow, ByVal row2 As PageRolerow) As Boolean
			Return True
		End Function
		...
		Public Shared Sub Updated(ByVal row1 As PageRolerow, ByVal row2 As PageRolerow)
		...
		Public Shared Function Deleting(ByVal rows As Collection(Of PageRolerow)) As Boolean
			Return True
		End Function
		...
		Public Shared Sub Deleted(ByVal rows As Collection(Of PageRolerow))
			Dim i As Integer = 0
			Do While (i < rows.Count)
				WriteAuditTrail(DirectCast(rows(i), PageRolerow), Nothing, Core.PageType.Delete)
				i += 1
			Loop
		End Sub
		...
		Private Shared Sub WriteAuditTrail(ByVal row1 As PageRolerow, ByVal row2 As PageRolerow, ByVal pt As Core.PageType)
			...
			Dim strTable As String = "PageRole"
			...
			Dim info As PageRoleinf = New PageRoleinf()
			...
			If (row1 IsNot Nothing) Then strKeys &= row1.PageRoleID.ToString() Else strKeys &= row2.PageRoleID.ToString()
