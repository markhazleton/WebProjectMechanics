...
Imports EW.Web
Namespace PMGEN
	Public MustInherit class PageRoledal_base
		...
		Dim objAdvancedSearchParm1 As PageRolerow ' Advanced Search Parm 1
		Dim objAdvancedSearchParm2 As PageRolerow ' Advanced Search Parm 2
		...
		Public Sub New (ByVal isUseSession As Boolean)
			...
			If (bUseSession) Then
				...
				' Advanced Search arguments 
				Dim sSearchName As String = Share.ProjectName + "_" + PageRoleinf.TableVar + "_Search1" 
				objAdvancedSearchParm1 = LoadSearch(sSearchName) 
				sSearchName = Share.ProjectName + "_" + PageRoleinf.TableVar + "_Search2" 
				objAdvancedSearchParm2 = LoadSearch(sSearchName) 
			Else
				objAdvancedSearchParm1 = New PageRolerow()
				objAdvancedSearchParm2 = New PageRolerow()
			End If
		End Sub
		...
		Public ReadOnly Property AdvancedSearchParm1 As PageRolerow 
			Get 
		...
		Public Readonly Property AdvancedSearchParm2 As PageRolerow 
			Get 
		...
		Private Sub SaveProperties()
			If (bUseSession) Then 
				...
				Dim sSearchName As String = String.Concat(Share.ProjectName, "_", PageRoleinf.TableVar, "_Search1") 
				SaveSearch(sSearchName, objAdvancedSearchParm1) 
				sSearchName = String.Concat(Share.ProjectName, "_", PageRoleinf.TableVar, "_Search2") 
		...
		Private Function LoadSearch(ByVal objectName As String) As PageRolerow 
			If (bUseSession) Then
				If (HttpContext.Current.Session(objectName) Is Nothing) Then
					HttpContext.Current.Session(objectName) = New PageRolerow()
				End If
				Return TryCast(HttpContext.Current.Session(objectName), PageRolerow)
			Else
				Return Nothing
			End If
		End Function
		...
		Private Sub SaveSearch(ByVal objectName As String, ByVal row As PageRolerow)
			If (bUseSession) Then
				If (HttpContext.Current.Session(objectName) Is Nothing) Then
					HttpContext.Current.Session(objectName) = New PageRolerow()
				End If
		...
		Private Function GetAdvancedSearch(ByVal row1 As PageRolerow, ByVal row2 As PageRolerow, ByRef parameters As DbParameterCollection ) As String 
			...
			If (bUseSession) Then
				...
				' PageRoleID Field 
				parm = Core.SetAdvancedSearchParm(row1.Attribute("PageRoleID").SearchType)
				strParmWhere.Remove(0, strParmWhere.Length)
				If (parm.Operator.Length > 0) Then
					strParmWhere.Append(String.Concat("([PageRoleID] ", parm.Operator, " ",  Database.GetSqlParameterName(Db.DbType, "AdvSearch1__PageRoleID"), ")"))
					oParameter = New OleDbParameter("AdvSearch1__PageRoleID", OleDbType.Integer)
					oParameter.Value = row1.PageRoleID
					parameters.Add(oParameter)
				End If
				parm = Core.SetAdvancedSearchParm(row2.Attribute("PageRoleID").SearchType)
				If (parm.Operator.Length > 0) Then
					If (strParmWhere.Length > 0) Then strParmWhere.Append(String.Concat(" ", row1.Attribute("PageRoleID").SearchCond.ToString(), " "))
					strParmWhere.Append(String.Concat("([PageRoleID] ", parm.Operator, " ",  Database.GetSqlParameterName(Db.DbType, "AdvSearch2__PageRoleID"), ")"))
					oParameter = New OleDbParameter("AdvSearch2__PageRoleID", OleDbType.Integer)
					oParameter.Value = row2.PageRoleID
					parameters.Add(oParameter)
				End If
				...
				' RoleID Field 
				parm = Core.SetAdvancedSearchParm(row1.Attribute("RoleID").SearchType)
				strParmWhere.Remove(0, strParmWhere.Length)
				If (parm.Operator.Length > 0) Then
					strParmWhere.Append(String.Concat("([RoleID] ", parm.Operator, " ",  Database.GetSqlParameterName(Db.DbType, "AdvSearch1__RoleID"), ")"))
					oParameter = New OleDbParameter("AdvSearch1__RoleID", OleDbType.Integer)
					oParameter.Value = row1.RoleID
					parameters.Add(oParameter)
				End If
				parm = Core.SetAdvancedSearchParm(row2.Attribute("RoleID").SearchType)
				If (parm.Operator.Length > 0) Then
					If (strParmWhere.Length > 0) Then strParmWhere.Append(String.Concat(" ", row1.Attribute("RoleID").SearchCond.ToString(), " "))
					strParmWhere.Append(String.Concat("([RoleID] ", parm.Operator, " ",  Database.GetSqlParameterName(Db.DbType, "AdvSearch2__RoleID"), ")"))
					oParameter = New OleDbParameter("AdvSearch2__RoleID", OleDbType.Integer)
					oParameter.Value = row2.RoleID
					parameters.Add(oParameter)
				End If
				If (strParmWhere.Length > 0) Then 
					If (strWhere.Length > 0) Then strWhere.Append(" AND ")
		...
		Public Overridable Function LoadRows(ByVal filter As String) As PageRolerow()
			Dim rows As PageRolerows = LoadList(filter)
			Dim array As PageRolerow() = Nothing
			If (rows IsNot Nothing) Then
				array = New PageRolerow(rows.Count - 1) {}
				rows.CopyTo(array, 0)
			End If
			Return array
		End Function
		...
		Private Function LoadDataViewFromSql(ByVal filter As String, ByVal orderBy As String, ByRef parameters As DbParameterCollection) As DataView 
			...
			Catch e As Exception
				...
				End If
				Throw New PageRoleDataException(strDbErrorMessage)
			End Try
		End Function
		...
		Public Overridable Function LoadList(ByVal filter As String) As PageRolerows 
			Dim parameters As DbParameterCollection = Db.GetParameterCollection()
			...
			Try
				...
				If (dv Is Nothing) Then
					Return Nothing
				End If
				Dim rows As PageRolerows = New PageRolerows()
				For Each drv As DataRowView In dv
					Dim row As PageRolerow = GetRow(drv)
					rows.Add(row)
				Next
		...
		Public Overridable Function LoadList(ByVal filter As String, ByVal PageSize As Integer, ByVal StartRow As Integer) As PageRolerows 
			Dim parameters As DbParameterCollection = Db.GetParameterCollection()
			Dim strWhere As String = SearchFilter(parameters) 
			Dim strMasterDetailWhere As String = MasterDetailFilter(parameters)
			If (Not String.IsNullOrEmpty(strMasterDetailWhere)) Then
		...
		Public Overridable Function LoadRow(ByVal key As PageRolekey, ByVal filter As String) As PageRolerow 
			Dim parameters As DbParameterCollection = Db.GetParameterCollection()
		...
		Private Function GetRow(ByVal drv As DataRowView) As PageRolerow ' Get a row based on data reader
			Dim row As PageRolerow = New PageRolerow()
			Try

				' Field PageRoleID
				If (Not DBNull.Value.Equals(drv("PageRoleID"))) Then
					row.PageRoleID = Convert.ToInt32(drv("PageRoleID"))
				Else
					row.PageRoleID = Nothing
				End If
		...
		Public Overridable Sub Update(ByVal row As PageRolerow)
			Dim oParameter As OleDbParameter = Nothing
			...
			Try
				Using oConn As DbConnection = Db.GetConnection()
					Using oCmd As DbCommand = Db.GetCommand(sSql, oConn)
						If (row.Attribute("PageRoleID").AllowUpdate) Then
							oParameter = New OleDbParameter("PageRoleID", OleDbType.Integer)
							If (row.PageRoleID.HasValue) Then
								oParameter.Value = DirectCast(row.PageRoleID, Object)
							Else
						...
						' Key parameters
						oParameter = New OleDbParameter("PageRoleID", OleDbType.Integer)
						oParameter.Value = row.PageRoleID
		...
		Public Overridable Function Insert(ByVal row As PageRolerow) As PageRolerow 
			...
			Using oConn As DbConnection = Db.GetConnection()
				...
				Using oCmd As DbCommand = Db.GetCommand(sSql, oConn)
					Try
						...
						strDbErrorMessage = String.Empty
						bStatus = True ' Successful
						row.PageRoleID = Db.LastInsertedID(oConn) ' load auto-generated ID
		...
		Public Sub Delete(ByVal PageRoleID As Int32) 
			Dim oParameter As OleDbParameter = Nothing
			Dim bStatus As Boolean 
		...
		Public Sub Delete(ByVal row As PageRolerow )
			Me.Delete(CType(row.PageRoleID, Int32))
		End Sub
		...
		Public Overridable Function KeyFilter(ByVal keys As ArrayList) As String 
			...
			Do While (i < keys.Count)
				...
				strWhere.Append(String.Concat("(", KeyFilter(DirectCast(keys(i), PageRolekey)), ")"))
				i += 1
			Loop
			Return strWhere.ToString()
		End Function
		...
		Public Overridable Function KeyFilter(ByVal keys As ArrayList, ByRef parameters As DbParameterCollection) As String 
			...
			Do While (i < keys.Count)
				If (strWhere.Length > 0) Then strWhere.Append(" OR ") 
				strWhere.Append(String.Concat("(", KeyFilter(DirectCast(keys(i), PageRolekey), parameters, i), ")"))
				i += 1
			Loop
			Return strWhere.ToString()
		End Function
		...
		Public Overridable Function KeyFilter(ByVal key As PageRolekey) As String 
			Dim strWhere As StringBuilder = New StringBuilder()

			' Key parameters
			If (strWhere.Length > 0) Then strWhere.Append(" AND ")
			strWhere.Append(String.Concat("[PageRoleID] = ", key.PageRoleID)) ' [PageRoleID] field
			Return strWhere.ToString()
		End Function
		...
		Public Overridable Function KeyFilter(ByVal key As PageRolekey, ByRef parameters As DbParameterCollection) As String 
			Dim strWhere As StringBuilder = New StringBuilder()
			Dim oParameter As OleDbParameter = Nothing

			' Key parameters
			If (strWhere.Length > 0) Then strWhere.Append(" AND ")
			strWhere.Append("[PageRoleID] = " & Database.GetSqlParameterName(Db.DbType, "key__PageRoleID"))
			oParameter = New OleDbParameter("key__PageRoleID", OleDbType.Integer)
			oParameter.Value = key.PageRoleID
			parameters.Add(oParameter)
			Return strWhere.ToString()
		End Function
		...
		Public Overridable Function KeyFilter(ByVal key As PageRolekey, ByRef parameters As DbParameterCollection, ByVal index As Integer) As String
			Dim strWhere As StringBuilder = New StringBuilder()
			Dim oParameter As OleDbParameter = Nothing

			' Key parameters
			If (strWhere.Length > 0) Then strWhere.Append(" AND ")
			strWhere.Append("[PageRoleID] = " & Database.GetSqlParameterName(Db.DbType, "key__PageRoleID" & index.ToString())) ' [PageRoleID] field
			oParameter = New OleDbParameter("key__PageRoleID" + index.ToString(), OleDbType.Integer)
			oParameter.Value = key.PageRoleID
			parameters.Add(oParameter)
			Return strWhere.ToString()
		End Function
		...
		Public Class PageRoleDataException 
			Inherits Exception
