...
Namespace PMGEN
	...
	''' <summary>
	''' Summary description for Articlebll
	''' Bussiness Layer (Article)
	''' </summary>

	Public MustInherit Class Articlebll_base
		Public Sub New() 
		End Sub
		...
		Public Shared Function Inserting(ByVal row1 As Articlerow) As Boolean
			Return True
		End Function
		...
		Public Shared Sub Inserted(ByVal row1 As Articlerow)
		...
		Public Shared Function Updating(ByVal row1 As Articlerow, ByVal row2 As Articlerow) As Boolean
			Return True
		End Function
		...
		Public Shared Sub Updated(ByVal row1 As Articlerow, ByVal row2 As Articlerow)
		...
		Public Shared Function Deleting(ByVal rows As Collection(Of Articlerow)) As Boolean
			Return True
		End Function
		...
		Public Shared Sub Deleted(ByVal rows As Collection(Of Articlerow))
			Dim i As Integer = 0
			Do While (i < rows.Count)
				WriteAuditTrail(DirectCast(rows(i), Articlerow), Nothing, Core.PageType.Delete)
				i += 1
			Loop
		End Sub
		...
		Private Shared Sub WriteAuditTrail(ByVal row1 As Articlerow, ByVal row2 As Articlerow, ByVal pt As Core.PageType)
			...
			Dim strTable As String = "Article"
			...
			If (row1 IsNot Nothing) Then strKeys &= row1.ArticleID.ToString() Else strKeys &= row2.ArticleID.ToString()
