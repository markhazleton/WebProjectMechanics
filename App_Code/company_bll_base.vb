...
Namespace PMGEN
	...
	''' Bussiness Layer (Company)
	''' </summary>

	Public MustInherit Class Companybll_base
		Public Sub New() 
		End Sub
		...
		Public Shared Function Inserting(ByVal row1 As Companyrow) As Boolean
			Return True
		End Function
		...
		Public Shared Sub Inserted(ByVal row1 As Companyrow)
		...
		Public Shared Function Updating(ByVal row1 As Companyrow, ByVal row2 As Companyrow) As Boolean
			Return True
		End Function
		...
		Public Shared Sub Updated(ByVal row1 As Companyrow, ByVal row2 As Companyrow)
		...
		Public Shared Function Deleting(ByVal rows As Collection(Of Companyrow)) As Boolean
			Return True
		End Function
		...
		Public Shared Sub Deleted(ByVal rows As Collection(Of Companyrow))
			Dim i As Integer = 0
			Do While (i < rows.Count)
				WriteAuditTrail(DirectCast(rows(i), Companyrow), Nothing, Core.PageType.Delete)
				i += 1
			Loop
		End Sub
		...
		Private Shared Sub WriteAuditTrail(ByVal row1 As Companyrow, ByVal row2 As Companyrow, ByVal pt As Core.PageType)
			...
			Dim strTable As String = "Company"
			...
			If (row1 IsNot Nothing) Then strKeys &= row1.CompanyID.ToString() Else strKeys &= row2.CompanyID.ToString()
