...
Namespace PMGEN
	...
	''' <summary>
	''' Summary description for SiteParameterTypebll
	''' Bussiness Layer (SiteParameterType)
	''' </summary>

	Public MustInherit Class SiteParameterTypebll_base
		Public Sub New() 
		End Sub
		...
		Public Shared Function Inserting(ByVal row1 As SiteParameterTyperow) As Boolean
			Return True
		End Function
		...
		Public Shared Sub Inserted(ByVal row1 As SiteParameterTyperow)
		...
		Public Shared Function Updating(ByVal row1 As SiteParameterTyperow, ByVal row2 As SiteParameterTyperow) As Boolean
			Return True
		End Function
		...
		Public Shared Sub Updated(ByVal row1 As SiteParameterTyperow, ByVal row2 As SiteParameterTyperow)
		...
		Public Shared Function Deleting(ByVal rows As Collection(Of SiteParameterTyperow)) As Boolean
			Return True
		End Function
		...
		Public Shared Sub Deleted(ByVal rows As Collection(Of SiteParameterTyperow))
			Dim i As Integer = 0
			Do While (i < rows.Count)
				WriteAuditTrail(DirectCast(rows(i), SiteParameterTyperow), Nothing, Core.PageType.Delete)
				i += 1
			Loop
		End Sub
		...
		Private Shared Sub WriteAuditTrail(ByVal row1 As SiteParameterTyperow, ByVal row2 As SiteParameterTyperow, ByVal pt As Core.PageType)
			...
			Dim strTable As String = "SiteParameterType"
			...
			If (row1 IsNot Nothing) Then strKeys &= row1.SiteParameterTypeID.ToString() Else strKeys &= row2.SiteParameterTypeID.ToString()
