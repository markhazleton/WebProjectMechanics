...
<XmlRootAttribute("WebsiteProfile", _
 Namespace:="http://www.projectmechanics.com", IsNullable:=False)> _
...
#End Region
    ...
    Public Function BuildPageTree(ByVal ParentID As String, ByVal intLevel As Double, ByVal sULClassName As String) As String
        Return LocationList.BuildPageTree(ParentID, intLevel, sULClassName)
    End Function
...
#Region "Private Methods"
    ...
    Private Function GetIndexFilePath() As String
        ...
        End If
        Return Replace(wpmApp.Config.ConfigFolderPath & "\index\" & Me.CompanyName & "-site-file.xml", "\\", "\")
    End Function
    ...
    Private Function SaveSiteMapFile() As Boolean
        ...
        Try
            Using sw As New System.IO.StreamWriter(GetIndexFilePath(), False)
                Try
                    Dim ViewWriter As New System.Xml.Serialization.XmlSerializer(GetType(WebsiteProfile))
                    ViewWriter.Serialize(sw, Me)
                Catch ex As Exception
                    wpmLogging.AuditLog("Error Saving File - " & ex.ToString, "wpmSiteProfile.SaveSiteMapFile")
                    bReturn = False
                End Try
            End Using
        Catch ex As Exception
            wpmLogging.AuditLog("Error Before Saving File  - " & ex.ToString, "wpmSiteProfile.SaveSiteMapFile")
            bReturn = False
        End Try
