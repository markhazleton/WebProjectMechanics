        ...
        Using mydt As DataTable = ApplicationDAL.GetImageByID(myImageID)
            If mydt.Rows.Count = 1 Then
                For Each myrow As DataRow In mydt.Rows
                    ...
                    Medium = wpm_GetDBString(myrow("Medium"))
                    Size = wpm_GetDBString(myrow("Size"))
                    Price = wpm_GetDBString(myrow("Price"))
                    Color = wpm_GetDBString(myrow("Color"))
                    Subject = wpm_GetDBString(myrow("Subject"))
                    Sold = wpm_GetDBBoolean(myrow("Sold"))
            ...
            Using connection As New OleDbConnection(wpm_SQLDBConnString)
                Try
                    ...
                    Using command As New OleDbCommand("UPDATE [image] set " & _
                                ...
                                "[image].[Sold]=@Sold, [image].[ContactID]=@ContactID, [image].[ModifiedDT]=now(), [image].[ImageDate]=@ImageDate " & _
