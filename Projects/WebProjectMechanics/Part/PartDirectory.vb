...
Public Class PartDirectory
    ...
    Public Shared Function Rankimg(ByVal intRank As Integer) As String
        If intRank = 0 Then
            Return String.Format("<img border=""0"" alt=""This link is ranked a {0}"" src=""{1}link/image/rank0.gif"" width=""40"" height=""6"">", intRank, wpm_SiteConfig.ApplicationHome())
        Else
            Return String.Format("<img border=""0"" alt=""This link is ranked a {0}"" src=""{1}link/image/rank{0}.gif"" width=""40"" height=""6"">", intRank, wpm_SiteConfig.ApplicationHome())
        End If
    End Function
    ...
    Sub DrawAdminCategory()
        ...
        For Each myCat As PartCategory In ActiveCompany.LinkCategoryList
            If wpm_IsBreak(i, 2) Then
                ...
                sbRight.Append(String.Format("<A href=""/admin/maint/default.aspx?type=Part&PartID={0}"">Edit</A> <br />", myCat.ID))
            ...
            Else
                ...
                sbLeft.Append(String.Format("<A href=""/admin/maint/default.aspx?type=Part&PartID={0}"">Edit</A> <br />", myCat.ID))
    ...
    Sub DrawAdminLinks(ByVal CurrentCID As String)
        ECHO("<hr>")
        For Each myLink As Part In ActiveCompany.PartList
    ...
    Private Function GetCategoryLinks(ByVal CurrentCID As String) As String
        ...
        For Each myPart As Part In ActiveCompany.PartList
            If myPart.PartCategoryID = CurrentCID Then
                ...
                If myPart.PartSource = "SiteLink" Then
                    If myPart.LocationID = String.Empty Then
                        sbLinks.Append(String.Format("<li><span class=""highlight-one"">{0}</span>", sLocationInfo))
                    Else
                        sbLinks.Append(String.Format("<li><span class=""highlight-two"">{0}</span>", sLocationInfo))
                    End If
                    sbLinks.Append(String.Format(" [ <a href=""/admin/maint/default.aspx?type=Part&PartID={0}"">Edit</a>  ]</li>", myPart.PartID))
                Else
                    If myPart.LocationID = String.Empty Then
                        sbLinks.Append(String.Format("<li><span class=""highlight-three"">{0}</span>", sLocationInfo))
                    Else
                        sbLinks.Append(String.Format("<li><span class=""highlight-four"">{0}</span>", sLocationInfo))
                    End If
                    sbLinks.Append(String.Format(" [ <a href=""/admin/maint/default.aspx?type=Part&PartID={0}"">Edit</a>  ]</li>", myPart.PartID))
                End If
            End If
                    ...
                    If (j >= i) And (j < i + iLinksPerPage) Then
                        If Not IsDBNull(myLink.AmazonIndex) Then
                            If Trim(myLink.AmazonIndex) & "*" <> "*" Then
                                ECHO(String.Format("<a target=""_blank"" href=""http://www.amazon.com/exec/obidos/ASIN/{0}/thefrogsfolly-20?creative=327641&camp=14573&link_code=as1""><img border=""0"" src=""http://images.amazon.com/images/P/{0}.01.THUMBZZZ.jpg"" alt=""{1}""></a>", myLink.AmazonIndex, myLink.URL))
                            End If
                        End If
                    End If
                End If
    ...
    Sub DrawYUILinks(ByRef mySitemap As ActiveCompany)
        ...
        For Each myLink As Part In mySitemap.PartList
