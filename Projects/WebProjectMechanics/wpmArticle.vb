...
Public Class wpmArticle
    ...
    Public Function updateArticleBody() As Boolean
        ...
        Else
            Dim connection As New OleDbConnection(wpmApp.ConnStr)
            Try
                connection.Open()
                Using myCommand As New OleDbCommand("UPDATE [Article] set " & _
                    ...
                    myCommand.Parameters.AddWithValue("@ArticleBody", Me.ArticleBody)
                    myCommand.Parameters.AddWithValue("@ArticleName", Me.ArticleName)
                    myCommand.Parameters.AddWithValue("@ArticleID", Me.ArticleID)
                    If myCommand.ExecuteNonQuery() > 0 Then
                        result = True
                    End If
                End Using
            Catch ex As Exception
                wpmLogging.AuditLog("mhArticle.updateArticleBody", ex.ToString)
            Finally
                connection.Close()
                connection = Nothing
            End Try
    ...
    Private Function GetArticleAdmin(ByVal ArticleID As String) As String
        If wpmUser.IsAdmin() Then
            If (Me.IsArticleDefault) Then
                Return ("<br /><br /><hr /><font color=""red"">No Article For Current Page -> </font><a href=""" & wpmApp.Config.AspMakerGen & "article_add.aspx""> Add Article </a>")
            Else
                Return ("<br/><br /><hr /><a href=""" & wpmApp.Config.AspMakerGen & "article_edit.aspx?ArticleID=" & ArticleID & """> Edit Article </a>")
            End If
        Else
            Return ""
        End If
...
Public Class wpmArticleList
    ...
    Private Function GetFileName(ByVal FileName As String) As String
        Return wpmApp.Config.ConfigFolderPath & FileName & ".xml"
    End Function
    ...
    Private Function SaveArticleListFile(ByVal filename As String) As Boolean
        Try
            wpmFileProcessing.DeleteFile(GetFileName(filename))
            Using ViewFile As New System.IO.StreamWriter(GetFileName(filename))
    ...
    Private Function LoadArticleListFile(ByVal filename As String) As Boolean
        ...
        Try
            Using sr As New StreamReader(GetFileName(filename))
    ...
    Public Function GetBlogPosts(ByRef sbBlogTemplate As StringBuilder) As String
        ...
        Dim sWeblogRSS As String = ("http://" & HttpContext.Current.Request.ServerVariables.Item("SERVER_NAME") & "" & wpmApp.Config.wpmWebHome() & "blog/blog_rss.aspx?c=" & curPageID)
