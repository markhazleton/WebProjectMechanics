<%@ Page Language="VB" MasterPageFile="masterpage.master" ValidateRequest="false" AutoEventWireup="false" CodeFile="Link_edit.aspx.vb" Inherits="Link_edit" CodeFileBaseClass="AspNetMaker7_WPMGen" %>
...
<p><span class="aspnetmaker">Edit TABLE: Site Parts<br /><br />
<a href="<%= Link.ReturnUrl %>">Go Back</a></span></p>
<% Link_edit.ShowMessage() %>
<form name="fLinkedit" id="fLinkedit" method="post">
<p />
...
<input type="hidden" name="x_ID" id="x_ID" value="<%= ew_HTMLEncode(Link.ID.CurrentValue) %>" />
...
<select id="x_CompanyID" name="x_CompanyID" onchange="ew_UpdateOpt('x_zPageID','x_CompanyID',Link_edit.ar_x_zPageID);ew_UpdateOpt('x_UserID','x_CompanyID',Link_edit.ar_x_UserID);"<%= Link.CompanyID.EditAttributes %>>
<%
emptywrk = True
...
<% End If %>
<% If Link.UserName.Visible Then ' UserName %>
	<tr<%= Link.UserName.RowAttributes %>>
		<td class="ewTableHeader">User Name</td>
		<td<%= Link.UserName.CellAttributes %>><span id="el_UserName">
<input type="text" name="x_UserName" id="x_UserName" size="30" maxlength="50" value="<%= Link.UserName.EditValue %>"<%= Link.UserName.EditAttributes %> />
</span><%= Link.UserName.CustomMsg %></td>
	</tr>
<% End If %>
...
<p />
<input type="button" name="btnAction" id="btnAction" value="Save Changes" onclick="ew_SubmitForm(Link_edit, this.form);" />
</form>
<script language="JavaScript">
<!--
ew_UpdateOpts([['x_zPageID','x_CompanyID',Link_edit.ar_x_zPageID],['x_UserID','x_CompanyID',Link_edit.ar_x_UserID]]);
//-->
</script>
<script type="text/javascript">
<!--
