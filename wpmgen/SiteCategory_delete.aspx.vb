...
Partial Class SiteCategory_delete
	...
	Sub Page_Main()
		...
		For i As Integer = 0 to arRecKeys.GetUpperBound(0)
			...
			sFilter = sFilter & "[SiteCategoryID]=" & ew_AdjustSql(sKeyFld) & " AND "
    ...
    user = CurrentUserName()
		...
		' GroupOrder Field
		ew_WriteAuditTrail(filePfx, curDate, curTime, id, user, action, table, "GroupOrder", keyvalue, RsSrc("GroupOrder"), newvalue)

		' CategoryName Field
		ew_WriteAuditTrail(filePfx, curDate, curTime, id, user, action, table, "CategoryName", keyvalue, RsSrc("CategoryName"), newvalue)

		' CategoryTitle Field
		ew_WriteAuditTrail(filePfx, curDate, curTime, id, user, action, table, "CategoryTitle", keyvalue, RsSrc("CategoryTitle"), newvalue)

		' CategoryDescription Field
		ew_WriteAuditTrail(filePfx, curDate, curTime, id, user, action, table, "CategoryDescription", keyvalue, RsSrc("CategoryDescription"), newvalue)

		' CategoryKeywords Field
		ew_WriteAuditTrail(filePfx, curDate, curTime, id, user, action, table, "CategoryKeywords", keyvalue, RsSrc("CategoryKeywords"), newvalue)

		' ParentCategoryID Field
		ew_WriteAuditTrail(filePfx, curDate, curTime, id, user, action, table, "ParentCategoryID", keyvalue, RsSrc("ParentCategoryID"), newvalue)

		' CategoryFileName Field
		ew_WriteAuditTrail(filePfx, curDate, curTime, id, user, action, table, "CategoryFileName", keyvalue, RsSrc("CategoryFileName"), newvalue)
