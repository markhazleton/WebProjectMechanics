...
Partial Class SiteParameterType_delete
	...
	Sub Page_Main()
		...
		For i As Integer = 0 to arRecKeys.GetUpperBound(0)
			...
			sFilter = sFilter & "[SiteParameterTypeID]=" & ew_AdjustSql(sKeyFld) & " AND "
    ...
    user = CurrentUserName()
		...
		ew_WriteAuditTrail(filePfx, curDate, curTime, id, user, action, table, "SiteParameterTypeNM", keyvalue, RsSrc("SiteParameterTypeNM"), newvalue)

		' SiteParameterTypeDS Field
		ew_WriteAuditTrail(filePfx, curDate, curTime, id, user, action, table, "SiteParameterTypeDS", keyvalue, RsSrc("SiteParameterTypeDS"), newvalue)

		' SiteParameterTypeOrder Field
		ew_WriteAuditTrail(filePfx, curDate, curTime, id, user, action, table, "SiteParameterTypeOrder", keyvalue, RsSrc("SiteParameterTypeOrder"), newvalue)

		' SiteParameterTemplate Field
		ew_WriteAuditTrail(filePfx, curDate, curTime, id, user, action, table, "SiteParameterTemplate", keyvalue, "<MEMO>", newvalue)
	End Sub
