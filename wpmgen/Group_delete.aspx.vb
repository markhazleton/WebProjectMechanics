...
Partial Class Group_delete
	...
	Sub Page_Main()
		...
		For i As Integer = 0 to arRecKeys.GetUpperBound(0)
			...
			sFilter = sFilter & "[GroupID]=" & ew_AdjustSql(sKeyFld) & " AND "
    ...
    user = CurrentUserName()
		...
		' GroupName Field
		ew_WriteAuditTrail(filePfx, curDate, curTime, id, user, action, table, "GroupName", keyvalue, RsSrc("GroupName"), newvalue)

		' GroupComment Field
		ew_WriteAuditTrail(filePfx, curDate, curTime, id, user, action, table, "GroupComment", keyvalue, RsSrc("GroupComment"), newvalue)
	End Sub
