...
Public Class controls_DisplayTable
    ...
    Private Function FormatHeaderColumn(ByVal myColName As String) As String
        ...
        Select Case Right(myColName, 4)
            Case "Desc"
                newColumnName = Left(myColName, Len(myColName) - 4) & " Description"
            Case Else
                'do nothing
        End Select
            ...
            If myHeader.DetailFieldName <> String.Empty Then
                Try
                    If myHeader.DetailKeyGridIndex <= i.Value.Count AndAlso myHeader.DetailFieldGridIndex <= i.Value.Count Then
                        ...
                        If String.IsNullOrWhiteSpace(MyTest2) Then
                            MyTest2 = "empty"
                        End If
    ...
    Public Sub BuildTable(ByVal myHeader As DisplayTableHeader, ByVal myRows As Object) Implements Icontrols_DisplayTable.BuildTable
        ...
        myrow = New StringBuilder
        If myHeader.DetailFieldName <> String.Empty Then
            myrow.AppendLine(String.Format("<th {1}>{0}</th>", FormatHeaderColumn(myHeader.DetailFieldName), String.Empty))
        End If
        ...
        For Each i In myRows
            ...
            For Each p In myHeader.HeaderItems
                If p.Name = "Thumbnail" Then
                    myrow.AppendLine(String.Format("<td><a href='{0}' ><img width='50px' src='{2}{1}' alt='{1}' /></a></td>", String.Format(p.LinkPath, GetPropertyValue(i, p.LinkKeyName)), GetPropertyValue(i, p.LinkTextName).Replace(".jpg", ".jpg"), p.ThumbnailPath))
                Else
                    If p.KeyField Then
                    ...
                    Else
                        If p.DisplayFormat = DisplayFormat.YesNo Then
                            Dim myYesNotValue As Boolean = False
